...
| Term | Definition | Translations | Aliases to avoid | Appears in |
| ---- | ---------- | ------------ | ---------------- | ---------- |
| **targetEpoch** | The target eating time (millisecond epoch) the scheduler works backward from to determine each Grillstück's put-on moment. | Essenszeit (de) | | `src/lib/schemas/index.ts:22`, `src/lib/grillade/lifecycle.ts` (`effectiveTargetEpoch`) |
...
| **SyncQueueRow** *(implementation)* | A queued API write (POST, PATCH, or DELETE) persisted in IDB, awaiting flush once the device is online and authenticated. | pendingWrite | `src/lib/stores/db.ts:35`, `src/lib/sync/coordinator.ts` |
...
Four modules concentrate previously-scattered logic. Touch one file when changing the corresponding concern; do not reach around them.

| Seam | File | Owns |
| ---- | ---- | ---- |
| **Field mappers** | `src/lib/sync/mappers/` (frontend), `backend/grillmi/routes/_models.py` (backend) | Every camelCase ⇔ snake_case conversion, every `Date.parse` round-trip, every `findCutBySlug` reconstruction, and the `PlannedItem` vs `SessionItem` discrimination. Renaming `cookSeconds` is a one-file edit on each side. |
| **AccountAccess** | `backend/grillmi/services/account_access.py` | Every login, logout, password reset, set-password, list-sessions, revoke-session, delete-account, and admin invitation flow. HTTP routes in `auth.py` and CLIs `admin_init` / `admin_reset` are thin shells over this service. |
| **SyncCoordinator** | `src/lib/sync/coordinator.ts` | The public sync surface (`syncNow`, `enqueueWrite`, `subscribe`, `attachSync`) and the single-active-grillade, watermark, and missing-server-row invariants. The former `pull.ts`, `queue.ts`, and `pushGrillade.ts` live under `src/lib/sync/_adapters/` as dumb HTTP/IDB shims driven by the coordinator. |
| **GrilladeLifecycle** | `src/lib/grillade/lifecycle.ts` | The full Grillade state machine (plan mutators, session start variants, item mutators, timeline append, endSession). Takes ports for `persistence`, `push`, `scheduler`, `clock` so tests run without IDB or Svelte mounts. `grilladeStore.svelte.ts` is a thin runes view that wraps a single lifecycle instance. |
...
- `src/lib/grillade/lifecycle.ts` exposes `loadFromMenu(items)` (forwarded by `grilladeStore.svelte.ts`; called from `+page.svelte` and `chronik/+page.svelte` to load past-Grillade items into a fresh Grillade; rename to something like `loadFromTemplate` or `loadFromGrillade`).
