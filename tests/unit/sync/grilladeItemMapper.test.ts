...
describe('grilladeItemMapper', () => {
	...
	it('sessionItemToServer serializes alarm dismissal timestamps as ISO strings', () => {
		...
		expect(sessionItemToServer(item, 0).alarm_state).toEqual({
			...
			flip: null,
			ready: null,
			firedAt: { putOn: null, flip: null, ready: null },
		})
	})

	it('sessionItemToServer serializes alarmFired timestamps under firedAt', () => {
		const item: SessionItem = {
			...PLANNED,
			id: 's3',
			putOnEpoch: 1714579200000,
			...
			platedEpoch: null,
			alarmDismissed: { putOn: null, flip: null, ready: null },
			alarmFired: { putOn: 1714579200000, flip: null, ready: null },
		}
		expect(sessionItemToServer(item, 0).alarm_state).toEqual({
			putOn: null,
			flip: null,
			ready: null,
			firedAt: { putOn: '2024-05-01T16:00:00.000Z', flip: null, ready: null },
		})
	})
	...
	it('sessionFromServer hydrates alarmFired from server alarm_state.firedAt', () => {
		const future = Date.now() + 60 * 60 * 1000
		const startedAt = Date.now() - 30 * 1000
		const row: GrilladeRow = {
			id: 'g',
		...
		const session = sessionFromServer(row, rawItems, planned)
		expect(session.items[0].alarmFired).toEqual({
			putOn: Date.parse('2024-05-01T16:00:00.000Z'),
			flip: null,
			ready: null,
		})
		expect(session.items[1].alarmFired).toEqual({ putOn: null, flip: null, ready: null })
	})
