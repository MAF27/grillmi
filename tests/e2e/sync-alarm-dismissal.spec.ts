...
test.describe('sync alarm dismissal', () => {
	test('test_alarm_dismissal_propagates_across_devices', async ({ browser }) => {
		const email = uniqueEmail('alarm-sync')
		...
		const patch = await a.context.request.patch(
			...
			{
				data: { alarm_state: { flip: dismissedAt } },
				headers: { 'X-CSRFToken': a.csrfToken, 'Content-Type': 'application/json' },
			},
		)
