# Grillmi: Grilltiming-Referenz

Umfassende, mit Quellen belegte Referenz für die Grillmi-PWA. Jeder Abschnitt enthält Tabellen pro Cut mit Dicke, Garstufe, Hitzezone, Grilltemperatur, Zeit pro Seite, Gesamtzeit, Wendeanzahl, Ruhezeit und Hinweisen. Quellenangaben verwenden Inline-Tags wie `[Weber-DE]`, `[SE]` (Serious Eats), `[Migros-Grilltimer]`, `[Napoleon]`, `[Traeger]`, `[BBCGF]` (BBC Good Food), `[Grillportal]`, `[Meathead]` (AmazingRibs). Vollständige Links im Quellenverzeichnis am Ende.

## Annahmen

Alle Angaben unten setzen voraus:

- **Gasgrill oder Kugelgrill** mit funktionierendem Deckel; Deckel zu, ausser explizit anders vermerkt (z. B. sehr dünne Filets, zu bestreichende Würste).
- **Zwei-Zonen-Setup**: eine Seite direkt (alle Brenner an oder volles Kohlebett), eine Seite indirekt (Brenner aus oder ohne Kohle darunter). "Direkt hoch" entspricht 230–290 °C Rosttemperatur, "direkt mittel" 180–230 °C, "indirekt" 150–180 °C, sofern nicht anders angegeben.
- **Fleisch auf Raumtemperatur tempriert**: 30–45 Min. ausserhalb des Kühlschranks für Steaks, 20–30 Min. für Koteletts, bis 60 Min. für Braten. Kaltes Fleisch verlängert die Zeit um 20–30 % und führt zu ungleichmässigem Garen. [SE, Meathead]
- **Lebensmittel leicht ölen, nicht den Rost**: Öl auf dem Rost verbrennt und klebt; das Grillgut ölen. [Weber-DE]
- **Sauberer, heisser Rost**: 10–15 Min. mit geschlossenem Deckel vorheizen, bevor etwas darauf kommt.
- **Kerntemperatur ist die Wahrheit**. Zeitangaben sind Richtwerte; bei Zielkerntemperatur mit Einstech-Thermometer (Thermapen-Klasse) ziehen. Alle Kerntemperaturen unten sind Pull-Temperaturen, das Nachgaren bringt 2–5 °C bei Steaks und 3–7 °C bei Braten während der Ruhezeit dazu. [SE, Meathead]
- **Wendeanzahl**: "einmal wenden" ist der Serious-Eats-Standard für Steaks ab 3 cm; Weber empfiehlt zunehmend, alle 45–60 s zu wenden für gleichmässigere Kruste und gleichmässigeres Garen (Kenji hat das empirisch gezeigt). Beides funktioniert. Der Grillmi-Standard ist **einmal wenden** für Cuts bis 3 cm und **alle 60 s wenden** für Cuts ab 4 cm oder bei Reverse-Sear-Finish.
- **Ruhezeit** gilt für jeden Cut, der ab 3 cm Dicke auf medium-rare oder mehr gegart wird. Locker mit Alufolie zelten; nicht straff einwickeln (dämpft die Kruste).
- **Schweizer/DACH-Cut-Bezeichnungen** folgen `[Migros-Grilltimer]` und `[Weber-DE]`. Wo ein Schweizer Cut keine saubere US-Entsprechung hat (Hohrücken, Huft, Nierstück), ist der deutsche Begriff primär.

## Interne Pull-Temperaturen (°C)

| Garstufe         | Rind / Lamm / Wild | Schwein | Geflügel (Brust) | Geflügel (Schenkel) | Fisch (meiste) | Thunfisch |
| ---------------- | ------------------ | ------- | ---------------- | ------------------- | -------------- | --------- |
| bleu / sehr rare | 46–48              | -       | -                | -                   | -              | 38–42     |
| rare             | 49–52              | -       | -                | -                   | -              | 43–46     |
| medium-rare      | 53–56              | 58–60   | -                | -                   | 48–52          | 47–50     |
| medium           | 57–60              | 62–65   | 70–72            | -                   | 55–58          | 52–55     |
| medium-well      | 61–65              | 66–68   | 73–74            | -                   | 60–63          | -         |
| well-done        | 66+                | 70+     | 75               | 80–85               | 65+            | -         |

Hinweis Schwein: Schweizer und US-Empfehlungen seit 2011 (USDA) erlauben ganzes Schweinemuskelfleisch (Filet, Schweinshals, Koteletts) bei 62–63 °C Pull mit Ruhezeit, Endtemperatur 65 °C. Ältere DACH-Empfehlungen nennen weiterhin 70 °C; Grillmi setzt standardmässig 63 °C Pull für ganzes Schweinemuskelfleisch und 70 °C für Hackfleisch und Schweinsbraten. [USDA]

Hinweis Geflügel: Brust ist sicher bei 70 °C Pull plus 3 Min. Ruhezeit (entspricht USDA 74 °C augenblicklich). Schenkel werden bei 80–85 °C gegessen, weil das Kollagen zum Schmelzen Hitze braucht. [SE, Meathead]

---

## Beef (Rind)

### Rinds-Entrecôte (Ribeye steak, boneless)

Fett, gutmütig. Der DACH-Steakklassiker. Zwei-Zonen-Reverse-Sear bevorzugt ab 4 cm, Einzonen direkt für 2–3 cm in Ordnung.

| Thickness | Doneness | Heat zone | Grate temp | Per side | Total | Turns | Rest |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 cm | rare | Direkt, Deckel zu | 230 °C | 1 min | 2 min | Einmal halbzeitig wenden | 3 min |
| 2 cm | rare | Direkt, Deckel zu | 230 °C | 2 min 15 s | 4 min 30 s | Einmal halbzeitig wenden | 3 min |
| 3 cm | rare | Direkt, Deckel zu | 230 °C | 3 min 30 s | 7 min | Einmal halbzeitig wenden | 3 min |
| 4 cm | rare | Direkt, Deckel zu | 230 °C | 6 min 30 s | 13 min | Einmal halbzeitig wenden | 5–8 min |
| 5 cm | rare | Reverse-Sear (indirekt zu direkt) | 230 °C | 7 min 30 s | 15 min | Einmal halbzeitig wenden | 8–10 min |
| 1 cm | medium-rare | Direkt, Deckel zu | 230 °C | 1 min 20 s | 2 min 40 s | Einmal halbzeitig wenden | 3 min |
| 2 cm | medium-rare | Direkt, Deckel zu | 230 °C | 2 min 45 s | 5 min 30 s | Einmal halbzeitig wenden | 3 min |
| 3 cm | medium-rare | Direkt, Deckel zu | 230 °C | 5 min | 10 min | Einmal halbzeitig wenden | 5 min |
| 4 cm | medium-rare | Direkt zu indirekt | 230 °C | 7 min 30 s | 15 min | Einmal halbzeitig wenden | 5–8 min |
| 5 cm | medium-rare | Reverse-Sear (indirekt zu direkt) | 230 °C | 9 min | 18 min | Einmal halbzeitig wenden | 8–10 min |
| 1 cm | medium | Direkt, Deckel zu | 230 °C | 2 min | 4 min | Einmal halbzeitig wenden | 3 min |
| 2 cm | medium | Direkt, Deckel zu | 230 °C | 3 min 45 s | 7 min 30 s | Einmal halbzeitig wenden | 3 min |
| 3 cm | medium | Direkt, Deckel zu | 230 °C | 6 min | 12 min | Einmal halbzeitig wenden | 5 min |
| 4 cm | medium | Direkt zu indirekt | 230 °C | 9 min | 18 min | Einmal halbzeitig wenden | 5–8 min |
| 5 cm | medium | Reverse-Sear (indirekt zu direkt) | 230 °C | 10 min | 20 min | Einmal halbzeitig wenden | 8–10 min |
| 1 cm | well-done | Direkt, Deckel zu | 230 °C | 3 min 30 s | 7 min | Einmal halbzeitig wenden | 3 min |
| 2 cm | well-done | Direkt, Deckel zu | 230 °C | 8 min | 16 min | Einmal halbzeitig wenden | 5 min |
| 3 cm | well-done | Direkt, Deckel zu | 230 °C | 8 min 30 s | 17 min | Einmal halbzeitig wenden | 5 min |
| 4 cm | well-done | Direkt zu indirekt | 230 °C | 14 min | 28 min | alle 2–3 min drehen | 5–8 min |
| 5 cm | well-done | Reverse-Sear (indirekt zu direkt) | 230 °C | 15 min | 30 min | alle 2–3 min drehen | 8–10 min |

Sources: [Migros-Grilltimer].

Notes:

- Grosszügig (flockig) 45+ Min. vor dem Grillen salzen (Dry-Brine) oder direkt davor; nie im Fenster zwischen 10 und 40 Min. [SE]
- Steak nicht andrücken. Nicht bewegen, bis es sich vom Rost löst (etwa 90 s beim ersten Kontakt jeder Seite).
- Karierte Grillmuster: nach der Hälfte jeder Seite um 45° drehen. Nur kosmetisch.

### Rinds-Filet (Beef tenderloin / filet mignon)

Mager, weniger fehlerverzeihend als das Entrecôte. 2–3 °C früher ziehen als gedacht.

| Thickness | Doneness | Heat zone | Grate temp | Per side | Total | Turns | Rest |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 cm | rare | Direkt, Deckel zu | 230 °C | 1 min | 2 min | Einmal halbzeitig wenden | 3 min |
| 2 cm | rare | Direkt, Deckel zu | 230 °C | 2 min 30 s | 5 min | Einmal halbzeitig wenden | 3 min |
| 3 cm | rare | Direkt, Deckel zu | 230 °C | 3 min 30 s | 7 min | Einmal halbzeitig wenden | 3 min |
| 4 cm | rare | Direkt, Deckel zu | 230 °C | 6 min 30 s | 13 min | Einmal halbzeitig wenden | 5–8 min |
| 5 cm | rare | Reverse-Sear (indirekt zu direkt) | 230 °C | 7 min 30 s | 15 min | Einmal halbzeitig wenden | 8–10 min |
| 1 cm | medium-rare | Direkt, Deckel zu | 230 °C | 1 min 15 s | 2 min 30 s | Einmal halbzeitig wenden | 3 min |
| 2 cm | medium-rare | Direkt, Deckel zu | 230 °C | 3 min 30 s | 7 min | Einmal halbzeitig wenden | 3 min |
| 3 cm | medium-rare | Direkt, Deckel zu | 230 °C | 5 min | 10 min | Einmal halbzeitig wenden | 5 min |
| 4 cm | medium-rare | Direkt zu indirekt | 230 °C | 7 min 30 s | 15 min | Einmal halbzeitig wenden | 5–8 min |
| 5 cm | medium-rare | Reverse-Sear (indirekt zu direkt) | 230 °C | 8 min 30 s | 17 min | Einmal halbzeitig wenden | 8–10 min |
| 1 cm | medium | Direkt, Deckel zu | 230 °C | 2 min | 4 min | Einmal halbzeitig wenden | 3 min |
| 2 cm | medium | Direkt, Deckel zu | 230 °C | 4 min 30 s | 9 min | Einmal halbzeitig wenden | 3 min |
| 3 cm | medium | Direkt, Deckel zu | 230 °C | 6 min | 12 min | Einmal halbzeitig wenden | 5 min |
| 4 cm | medium | Direkt zu indirekt | 230 °C | 9 min | 18 min | Einmal halbzeitig wenden | 5–8 min |
| 5 cm | medium | Reverse-Sear (indirekt zu direkt) | 230 °C | 9 min 30 s | 19 min | Einmal halbzeitig wenden | 8–10 min |
| 1 cm | well-done | Direkt, Deckel zu | 230 °C | 3 min 30 s | 7 min | Einmal halbzeitig wenden | 3 min |
| 2 cm | well-done | Direkt, Deckel zu | 230 °C | 8 min | 16 min | Einmal halbzeitig wenden | 5 min |
| 3 cm | well-done | Direkt, Deckel zu | 230 °C | 8 min 30 s | 17 min | Einmal halbzeitig wenden | 5 min |
| 4 cm | well-done | Direkt zu indirekt | 230 °C | 14 min | 28 min | alle 2–3 min drehen | 5–8 min |
| 5 cm | well-done | Reverse-Sear (indirekt zu direkt) | 230 °C | 14 min | 28 min | alle 2–3 min drehen | 8–10 min |

Sources: [Migros-Grilltimer].

Notes:

- Mit neutralem Öl bestreichen, salzen, frisch gemahlenen schwarzen Pfeffer. Optional in den letzten 60 s mit Butter arrosieren.
- Locker zelten lassen, niemals früh anschneiden, der ganze Saft steht unter leichtem Druck.

### Hohrücken (Rib steak, bone-in) / Rinds-Ribeye bone-in

Dicker als entbeintes Entrecôte; der Knochen isoliert eine Seite.

| Thickness | Doneness  | Heat zone          | Grate temp | Per side   | Total  | Turns                    | Rest    |
| --------- | --------- | ------------------ | ---------- | ---------- | ------ | ------------------------ | ------- |
| 2 cm      | medium    | Direkt, Deckel zu  | 230 °C     | 4 min 30 s | 9 min  | Einmal halbzeitig wenden | 3 min   |
| 4 cm      | medium    | Direkt zu indirekt | 230 °C     | 9 min      | 18 min | Einmal halbzeitig wenden | 5–8 min |
| 2 cm      | well-done | Direkt, Deckel zu  | 230 °C     | 8 min      | 16 min | Einmal halbzeitig wenden | 5 min   |
| 4 cm      | well-done | Direkt zu indirekt | 230 °C     | 9 min      | 18 min | Einmal halbzeitig wenden | 5–8 min |

Sources: [Migros-Grilltimer].

Notes:

- Fettkappe zuerst auslassen: 60 s mit der Fettseite nach unten anbraten, bevor der Hauptsear startet.
- 2 °C früher ziehen, wenn der Knochen dick ist; er gart während der Ruhezeit aggressiv weiter.

### T-Bone / Porterhouse

Zwei Muskeln, zwei Garzeiten: die Striploin-Seite ist schneller fertig als die Filet-Seite. Filet zur kühleren Zone ausrichten.

| Thickness | Doneness    | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest    |
| --------- | ----------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ------- |
| 4 cm      | medium-rare | Direkt, Deckel zu | 240 °C     | 4 min    | 8 min  | Einmal halbzeitig wenden | 5–8 min |
| 4 cm      | medium      | Direkt, Deckel zu | 240 °C     | 6 min    | 12 min | Einmal halbzeitig wenden | 5–8 min |

Sources: [Migros-Grilltimer].

Notes:

- Vor dem Anrichten 60 s auf der Knochenkante stehen lassen, um die Filet-Seite nachzuziehen, falls sie hinterherhinkt.
- Auf einem Schneidebrett ruhen lassen, nicht auf einem Teller (der Teller staut den Dampf unter dem Knochen).

### Bavette (Flanksteak / Flank steak)

Dünn, lang, mit grober Faser. Auf medium-rare essen, dünn quer zur Faser schneiden, sonst zäh.

| Thickness | Doneness    | Heat zone   | Grate temp | Per side | Total    | Turns         | Rest    |
| --------- | ----------- | ----------- | ---------- | -------- | -------- | ------------- | ------- |
| 1.5–2 cm  | medium-rare | Direkt hoch | 260–290 °C | 3–4 min  | 6–8 min  | Einmal wenden | 5–8 min |
| 2–2.5 cm  | medium-rare | Direkt hoch | 260–290 °C | 4–5 min  | 8–10 min | Einmal wenden | 8 min   |

Sources: [SE] 54 °C Kerntemperatur, 6–10 min total. [Weber-DE] 6–8 min.

Notes:

- In etwa 5 mm dicke Streifen quer zur Faser schneiden; vor dem Garen Faserrichtung identifizieren, sie ist offensichtlich.
- Niemals über medium hinaus, Kollagen hat in einem dünnen Steak keine Zeit zum Schmelzen.

### Bürgermeisterstück (Tri-tip)

Dreieckig, Faser wechselt mitten im Cut die Richtung. Reverse-Sear ist am besten.

| Thickness / weight | Doneness | Heat zone | Grate temp | Timing | Total | Turns | Rest |
| --- | --- | --- | --- | --- | --- | --- | --- |
| ~700 g, 4–5 cm dick in der Mitte | medium-rare | Reverse-Sear | 130 zu 290 °C | 25–35 ind + 2/Seite Sear | 30–45 min | alle 60 s drehen direkt | 10 min |
| ~1 kg | medium-rare | Reverse-Sear | 130 zu 290 °C | 35–45 ind + 2/Seite Sear | 40–55 min | alle 60 s drehen direkt | 10–15 min |

Sources: [SE] "Santa-Maria-Style", indirekt bis 49 °C, Sear bis 54 °C. [Traeger] 130 °C Smoke bis 52 °C, dann Sear. [Weber-US] stimmt zu.

Notes:

- Bei 52 °C für medium-rare ziehen (das Nachgaren ist bei einem 1-kg-Stück real).
- Dünn schneiden, am Scheitel den Faserwechsel beachten und für die zweite Hälfte des Bratens umorientieren.

### Hüftsteak (Rump / Huft)

Magerer als Entrecôte, gut marmorierte Varianten existieren. Wie ein etwas weniger fehlerverzeihendes Entrecôte behandeln.

| Thickness | Doneness    | Heat zone    | Grate temp    | Per side           | Total     | Turns                   | Rest  |
| --------- | ----------- | ------------ | ------------- | ------------------ | --------- | ----------------------- | ----- |
| 2 cm      | medium-rare | Direkt hoch  | 260–290 °C    | 2–3 min            | 4–6 min   | Einmal wenden           | 3 min |
| 3 cm      | medium-rare | Direkt hoch  | 260–290 °C    | 3–4 min            | 6–8 min   | Einmal wenden           | 5 min |
| 4 cm      | medium-rare | Reverse-Sear | 130 zu 290 °C | 12 ind + 1.5/Seite | 15–18 min | alle 60 s drehen direkt | 6 min |

Sources: [Weber-DE] stimmt zu.

### Rumpsteak (Sirloin / Striploin / New York strip)

Liegt geschmacklich nahe am Entrecôte, ist aber auf einer Seite magerer mit einem Fettstreifen auf der anderen.

| Thickness | Doneness    | Heat zone    | Grate temp    | Per side           | Total     | Turns                   | Rest  |
| --------- | ----------- | ------------ | ------------- | ------------------ | --------- | ----------------------- | ----- |
| 2.5 cm    | medium-rare | Direkt hoch  | 260–290 °C    | 3 min              | 6 min     | Einmal wenden           | 4 min |
| 3 cm      | medium-rare | Direkt hoch  | 260–290 °C    | 3.5–4 min          | 7–8 min   | Einmal wenden           | 5 min |
| 4 cm      | medium-rare | Reverse-Sear | 130 zu 290 °C | 12 ind + 1.5/Seite | 15–18 min | alle 60 s drehen direkt | 6 min |

Sources: [Weber-US] NY Strip 3 cm: 6–8 min. [SE] stimmt zu.

Notes:

- Den Fettstreifen vor dem Grillen an 3 Stellen einritzen, damit das Steak sich nicht wirft.
- Den Fettstreifen am Schluss 45–60 s hochkant anbraten.

### Roastbeef (ganzer Braten, Reverse-Sear)

Wird separat von Steaks behandelt, es ist ein Braten.

| Thickness | Doneness  | Heat zone         | Grate temp | Per side | Total  | Turns               | Rest     |
| --------- | --------- | ----------------- | ---------- | -------- | ------ | ------------------- | -------- |
| -         | medium    | Indirekt, niedrig | 200 °C     | 25 min   | 50 min | alle 2–3 min drehen | 8–10 min |
| -         | well-done | Indirekt, niedrig | 200 °C     | 30 min   | 60 min | alle 2–3 min drehen | 8–10 min |

Sources: [Migros-Grilltimer].

Notes:

- Bei 48–50 °C Kerntemperatur aus dem indirekten Bereich nehmen. 10 min ruhen lassen, während der Grill auf Maximum hochgeht. Allseitig anbraten. Erneut 15+ min ruhen lassen.
- Quer zur Faser in 3–5 mm dicke Scheiben schneiden.

---

## Veal (Kalb)

### Kalbs-Hohrücken (Veal rib steak)

Hell, delikat. Nie über medium, Kalb wird über 60 °C trocken.

| Thickness | Doneness  | Heat zone          | Grate temp | Per side   | Total  | Turns                    | Rest    |
| --------- | --------- | ------------------ | ---------- | ---------- | ------ | ------------------------ | ------- |
| 2 cm      | medium    | Direkt, Deckel zu  | 230 °C     | 4 min 30 s | 9 min  | Einmal halbzeitig wenden | 3 min   |
| 4 cm      | medium    | Direkt zu indirekt | 230 °C     | 9 min      | 18 min | Einmal halbzeitig wenden | 5–8 min |
| 2 cm      | well-done | Direkt, Deckel zu  | 230 °C     | 8 min      | 16 min | Einmal halbzeitig wenden | 5 min   |
| 4 cm      | well-done | Direkt zu indirekt | 230 °C     | 14 min     | 28 min | alle 2–3 min drehen      | 5–8 min |

Sources: [Migros-Grilltimer].

Notes:

- Nur Öl und Salz; Pfeffer erst nach dem Garen (verbrennt auf Kalb leicht).
- In den letzten 30 s mit Butter, Thymian und Knoblauch arrosieren, klassisch.

### Kalbs-Kotelett (Veal chop, bone-in)

| Thickness | Doneness  | Heat zone          | Grate temp | Per side    | Total  | Turns                    | Rest    |
| --------- | --------- | ------------------ | ---------- | ----------- | ------ | ------------------------ | ------- |
| 2 cm      | medium    | Direkt, Deckel zu  | 230 °C     | 5 min       | 10 min | Einmal halbzeitig wenden | 5 min   |
| 4 cm      | medium    | Direkt zu indirekt | 230 °C     | 9 min 30 s  | 19 min | Einmal halbzeitig wenden | 5–8 min |
| 2 cm      | well-done | Direkt, Deckel zu  | 230 °C     | 7 min 30 s  | 15 min | Einmal halbzeitig wenden | 5 min   |
| 4 cm      | well-done | Direkt zu indirekt | 230 °C     | 11 min 30 s | 23 min | Einmal halbzeitig wenden | 5–8 min |

Sources: [Migros-Grilltimer].

Notes:

- Am Ende 60 s auf den Knochen stellen für Aromaübertragung.
- Bei 60 °C ziehen, nach Ruhezeit 63 °C.

### Kalbshaxe (Veal shank, cross-cut / Beinscheibe / Osso buco)

Die ganze Haxe (1.0–1.5 kg) ist ein Schmorbraten und gehört in den Topf, nicht auf den Rost. Was auf den Grill kommt, ist die quergeschnittene Beinscheibe (Osso-buco-Schnitt) mit Knochen in der Mitte: 2–3 cm dick, direkt, in 15–20 min fertig. Beim Metzger als "Kalbsbeinscheibe" oder "Osso buco" bestellen.

| Thickness | Doneness  | Heat zone          | Grate temp | Per side | Total  | Turns                    | Rest  |
| --------- | --------- | ------------------ | ---------- | -------- | ------ | ------------------------ | ----- |
| 2 cm      | well-done | Direkt, Deckel zu  | 220 °C     | 7 min    | 14 min | Einmal halbzeitig wenden | 5 min |
| 3 cm      | well-done | Direkt zu indirekt | 220 °C     | 10 min   | 20 min | Einmal halbzeitig wenden | 5 min |

Sources: [Weber-Grill-Academy] "Beinscheibe / Osso buco direkt, 8–10 min pro Seite". [Meathead] AmazingRibs collagen guide (≥70 °C Kerntemperatur für zarte Beinscheibe).

Notes:

- Sehnen rundum mit der Spitze des Messers an 3–4 Stellen einschneiden, sonst zieht sich die Scheibe schüsselförmig zusammen.
- Salzen mindestens 30 min vorher.
- Kerntemperatur 65–70 °C — Beinscheiben sind erst ab dieser Temperatur richtig zart.
- Für die ganze Haxe: niedrig-und-langsam-Schmorbraten im Topf oder Smoker; kein direkter Grill.

---

## Pork (Schwein)

### Schweinsteak (Schulter oder Keule / Pork steak)

Meist Schulter; Budget-Cut, schmeckt am besten medium-well.

| Thickness | Doneness  | Heat zone          | Grate temp | Per side   | Total      | Turns                    | Rest    |
| --------- | --------- | ------------------ | ---------- | ---------- | ---------- | ------------------------ | ------- |
| 1 cm      | medium    | Direkt, Deckel zu  | 230 °C     | 1 min 30 s | 3 min      | Einmal halbzeitig wenden | 3 min   |
| 2 cm      | medium    | Direkt, Deckel zu  | 230 °C     | 3 min      | 6 min      | Einmal halbzeitig wenden | 3 min   |
| 3 cm      | medium    | Direkt, Deckel zu  | 230 °C     | 4 min      | 8 min      | Einmal halbzeitig wenden | 3 min   |
| 4 cm      | medium    | Direkt, Deckel zu  | 230 °C     | 5 min      | 10 min     | Einmal halbzeitig wenden | 5–8 min |
| 1 cm      | well-done | Direkt, Deckel zu  | 230 °C     | 2 min      | 4 min      | Einmal halbzeitig wenden | 3 min   |
| 2 cm      | well-done | Direkt, Deckel zu  | 230 °C     | 4 min 45 s | 9 min 30 s | Einmal halbzeitig wenden | 3 min   |
| 3 cm      | well-done | Direkt, Deckel zu  | 230 °C     | 6 min      | 12 min     | Einmal halbzeitig wenden | 5 min   |
| 4 cm      | well-done | Direkt zu indirekt | 230 °C     | 7 min 30 s | 15 min     | Einmal halbzeitig wenden | 5–8 min |

Sources: [Migros-Grilltimer].

Notes:

- Schweinefett würzt den Rost; auf Stichflammen achten und bei Bedarf in den indirekten Bereich verschieben.
- Marinierte Varianten: Hitze auf 200 °C reduzieren, Zucker verbrennt.

### Schweinsfilet (Pork tenderloin, whole)

Der Reverse-Sear-Star unter den Schweinecuts. Etwa 500 g pro Stück. NICHT über 63 °C Pull garen, es ist mager.

| Thickness | Doneness  | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest  |
| --------- | --------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ----- |
| -         | medium    | Direkt, Deckel zu | 230 °C     | 6 min      | 12 min | Einmal halbzeitig wenden | 5 min |
| -         | well-done | Direkt, Deckel zu | 230 °C     | 8 min 30 s | 17 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Silberhaut vor dem Grillen entfernen, sie löst sich nicht und wird ledrig.
- Wenn Zeit ist: 30 min in 5 %-Salzlösung pökeln, fantastisch.

### Schweinshals (Pork neck / collar / Schopfbraten steak)

Das Schweizer BBQ-Arbeitstier. Fett, aromatisch, gutmütig. Marinierte Varianten sind im DACH-Handel allgegenwärtig.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 230 °C     | 7 min    | 14 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Marinierter Schweinshals (das klassische Grillgut): bei direkt mittel-niedrig (180–200 °C) starten, damit die Marinade nicht verbrennt, dann auf direkt hoch 30–60 s/Seite für Farbe finishen.
- Bei zuckerreicher Marinade alle 60 s wenden.

### Gerollter Schweinshals (Rolled pork collar roast)

Wird als Braten behandelt.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns               | Rest     |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------- | -------- |
| -         | -        | Indirekt, niedrig | 230 °C     | 30 min   | 60 min | alle 2–3 min drehen | 8–10 min |

Sources: [Migros-Grilltimer].

### Schweinskoteletts (Pork chops, bone-in)

Magerer Rücken plus Knochen. Dünne trocknen sofort aus, wenn möglich ab 2.5 cm wählen.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 5 min    | 10 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- 30 min in 5 %-Salzlösung pökeln als Sicherheitsmarge.
- Am Ende 60 s auf der Knochenkante stehen lassen.

### Spareribs (Kotelettrippli / St. Louis)

Nur niedrig und langsam. Buchlanges Thema; hier die 3-2-1-Variante für den Gasgrill.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 230 °C     | 5 min    | 10 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Silberhaut auf der Knochenseite entfernen (mit einem Küchenpapier für Grip).
- Biegetest: ein Ende mit der Zange anheben; die Oberfläche soll Risse zeigen, wenn das Fleisch fertig ist.
- Glasur erst in den letzten 20–30 min, der Zucker in der Sauce verbrennt.

### Baby Back Ribs (Kotelettrippli, mager)

Kleiner und magerer als St. Louis. Kürzere Garzeit.

| Weight | Method | Heat zone | Grate temp | Timing | Total | Turns | Rest |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 Rack (~1 kg) | 2-2-1 | Indirekt | 110–120 °C | 2 h Smoke + 2 h gewickelt + 1 h glasiert | ~5 h | stündlich drehen | 10 min |
| 1 Rack | Indirekt ohne Wickeln | Indirekt | 135 °C | 2–3 h bis Biegetest | 2–3 h | alle 30 min drehen | 10 min |

Sources: [Weber-US] Baby Back 2-2-1. [Meathead] ähnlich.

---

## Lamb (Lamm)

### Lamm-Hüftli / Lamm-Hüftsteak (Lamb rump steak)

Schweizer Cut, üblicherweise 2–3 cm dick.

| Thickness | Doneness    | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest  |
| --------- | ----------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ----- |
| -         | rare        | Direkt, Deckel zu | 230 °C     | 3 min 30 s | 7 min  | Einmal halbzeitig wenden | 3 min |
| -         | medium-rare | Direkt, Deckel zu | 230 °C     | 4 min 30 s | 9 min  | Einmal halbzeitig wenden | 3 min |
| -         | medium      | Direkt, Deckel zu | 230 °C     | 6 min 30 s | 13 min | Einmal halbzeitig wenden | 5 min |
| -         | well-done   | Direkt, Deckel zu | 230 °C     | 8 min 30 s | 17 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

### Lamm-Koteletts (Lamb chops, bone-in)

Klein, schnell. Klassisch mit Rosmarin.

| Thickness | Doneness  | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest  |
| --------- | --------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ----- |
| -         | medium    | Direkt, Deckel zu | 230 °C     | 3 min 30 s | 7 min | Einmal halbzeitig wenden | 3 min |
| -         | well-done | Direkt, Deckel zu | 230 °C     | 4 min 30 s | 9 min | Einmal halbzeitig wenden | 3 min |

Sources: [Migros-Grilltimer].

Notes:

- Übermässiges Fett wegschneiden, um Stichflammen zu vermeiden.
- 30 min vorher mit Rosmarin, Knoblauch und Olivenöl würzen.

### Lamm-Nierstück (Lamb loin, saddle cuts)

| Thickness | Doneness    | Heat zone         | Grate temp | Per side   | Total       | Turns                    | Rest  |
| --------- | ----------- | ----------------- | ---------- | ---------- | ----------- | ------------------------ | ----- |
| 2 cm      | rare        | Direkt, Deckel zu | 230 °C     | 2 min      | 4 min       | Einmal halbzeitig wenden | 3 min |
| 3 cm      | rare        | Direkt, Deckel zu | 230 °C     | 3 min      | 6 min       | Einmal halbzeitig wenden | 3 min |
| 2 cm      | medium-rare | Direkt, Deckel zu | 230 °C     | 2 min 30 s | 5 min       | Einmal halbzeitig wenden | 3 min |
| 3 cm      | medium-rare | Direkt, Deckel zu | 230 °C     | 4 min      | 8 min       | Einmal halbzeitig wenden | 3 min |
| 2 cm      | medium      | Direkt, Deckel zu | 230 °C     | 3 min 15 s | 6 min 30 s  | Einmal halbzeitig wenden | 3 min |
| 3 cm      | medium      | Direkt, Deckel zu | 230 °C     | 5 min      | 10 min      | Einmal halbzeitig wenden | 5 min |
| 2 cm      | well-done   | Direkt, Deckel zu | 230 °C     | 4 min 15 s | 8 min 30 s  | Einmal halbzeitig wenden | 3 min |
| 3 cm      | well-done   | Direkt, Deckel zu | 230 °C     | 6 min 15 s | 12 min 30 s | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

### Lamm-Rack (Rack of lamb / Carré d'agneau)

Reverse-Sear ist der Move.

| Thickness | Doneness  | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest  |
| --------- | --------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ----- |
| -         | medium    | Direkt, Deckel zu | 230 °C     | 7 min    | 14 min | Einmal halbzeitig wenden | 5 min |
| -         | well-done | Direkt, Deckel zu | 230 °C     | 9 min    | 18 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Fettkappe vor dem Garen rautenförmig einritzen.
- Freiliegende Knochen mit Alufolie abdecken, damit sie nicht verbrennen.

### Lamm-Gigot (Leg of lamb, boneless or bone-in)

Klassisches Bratenterritorium.

| Thickness | Doneness | Heat zone         | Grate temp | Per side    | Total  | Turns               | Rest     |
| --------- | -------- | ----------------- | ---------- | ----------- | ------ | ------------------- | -------- |
| -         | -        | Indirekt, niedrig | 200 °C     | 27 min 30 s | 55 min | alle 2–3 min drehen | 8–10 min |

Sources: [Migros-Grilltimer].

Notes:

- Knoblauch- und Rosmarinstecken im Fleisch, klassisch.
- Aufgeklappt (butterflied) ist schneller und gart gleichmässiger.

---

## Horse (Pferd)

### Pferdesteak (Horse steak)

Sehr mager, sehr rot, gart SCHNELL, 2–3 °C früher ziehen.

| Thickness | Doneness    | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest  |
| --------- | ----------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ----- |
| -         | rare        | Direkt, Deckel zu | 230 °C     | 2 min      | 4 min  | Einmal halbzeitig wenden | 3 min |
| -         | medium-rare | Direkt, Deckel zu | 230 °C     | 3 min 30 s | 7 min  | Einmal halbzeitig wenden | 3 min |
| -         | medium      | Direkt, Deckel zu | 230 °C     | 5 min      | 10 min | Einmal halbzeitig wenden | 5 min |
| -         | well-done   | Direkt, Deckel zu | 230 °C     | 8 min      | 16 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- NICHT über medium-rare hinaus garen, Pferd hat etwa 3 % Fett und wird zum Stein.

---

## Poultry (Geflügel)

### Pouletschnitzel (Chicken breast, boneless skinless)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 6 min    | 12 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Dry-Brine (1 % Salz) für 1–24 h, riesiger Texturgewinn.
- Auf gleichmässige Dicke klopfen oder dicke Brüste aufschneiden (butterfly).

### Pouletschenkel / Pouletoberschenkel (Chicken thighs)

Ohne Knochen ohne Haut, ohne Knochen mit Haut oder mit Knochen und Haut.

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 8 min 30 s | 17 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Schenkel werden bei 80–85 °C gegessen, das Schmelzen des Kollagens ist der ganze Punkt. Nicht bei 72 °C aufhören, sonst werden sie gummig.
- Hautseite zuerst nach unten, nicht bewegen, bis sie sich löst.

### Poulet-Flügeli (Chicken wings)

Knusprige Haut ist das Ziel. Zwei Schulen: indirekt heiss-und-langsam, oder indirekt mit niedriger Temperatur und dann anbraten.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 250 °C     | 10 min   | 20 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- 1 TL Backpulver pro 500 g Flügeli erhöht den pH-Wert der Haut und verbessert die Knusprigkeit drastisch.
- Sauce NACH dem Garen, Sauce auf dem Grill ergibt verkohlten Schmodder.

### Aufgebrochenes Poulet (Spatchcock)

| Weight | Doneness | Heat zone | Grate temp | Timing | Total | Turns | Rest |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1.4–1.8 kg ganz | Brust 72 °C / Schenkel 82 °C | Indirekt, Brustseite oben | 200 °C | 45–60 min | 45–60 min | halbzeitig um 180° drehen | 10–15 min |
| 1.8–2.2 kg | Gleich | Indirekt, Brustseite oben | 200 °C | 60–75 min | 60–75 min | halbzeitig drehen | 15 min |

Sources: [SE] Spatchcock bei 230 °C 45 min. [Weber-DE] 200 °C 50–60 min.

Notes:

- Rückgrat mit Geflügelschere entfernen, flach pressen, halbiert die Garzeit gegenüber dem ganzen Brathuhn.
- 12–24 h im Kühlschrank trocken pökeln, die letzten 2 h offen für die Haut.

### Pouletspiessli (Chicken skewers)

| Piece size  | Doneness | Heat zone     | Grate temp | Per side  | Total     | Turns               | Rest  |
| ----------- | -------- | ------------- | ---------- | --------- | --------- | ------------------- | ----- |
| 3 cm Würfel | 72 °C    | Direkt mittel | 200 °C     | 2.5–3 min | 10–12 min | alle 2.5 min drehen | 2 min |

Sources: [Weber-DE] 8–10 min. [BBCGF] 10–12 min.

Notes:

- Holzspiesse 30 Min. wässern, sonst kohlen sie.
- Kleinen Spalt zwischen den Würfeln lassen, damit alle Seiten Hitze bekommen.

### Entenbrust (Duck breast)

Der Klassiker: Fett einritzen, mit der Hautseite auf einer kühlen Zone starten, Fett auslassen, heiss finishen.

| Thickness | Doneness  | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest  |
| --------- | --------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ----- |
| -         | medium    | Direkt, Deckel zu | 230 °C     | 5 min 30 s | 11 min | Einmal halbzeitig wenden | 5 min |
| -         | well-done | Direkt, Deckel zu | 230 °C     | 8 min 30 s | 17 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Fett rautenförmig einritzen, aber nicht ins Fleisch schneiden.
- Auf einer kühlen Zone starten, damit das Fett ausläuft, bevor die Haut verbrennt.

### Trutenschnitzel (Turkey cutlet)

Magerer als Pouletbrust, gleiche Regeln, aber gefährlicher, weil schneller trocken.

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total      | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | ---------- | ---------- | ------------------------ | ----- |
| 1 cm      | -        | Direkt, Deckel zu | 220 °C     | 2 min 45 s | 5 min 30 s | Einmal halbzeitig wenden | 3 min |
| 2 cm      | -        | Direkt, Deckel zu | 220 °C     | 5 min      | 10 min     | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Pökeln oder marinieren, trockene Trutenbrust ist unverzeihlich.

---

## Sausage (Wurst)

Allgemein: nie die Pelle platzen lassen. Das heisst indirekt mit niedriger Temperatur, bis sie durchgewärmt ist, dann direkt für Farbe. Bei vorgegarten Würsten (Cervelat, Wienerli, Bockwurst) ist es im Grunde nur "warm machen und Marken setzen".

### Bauernbratwurst

Rohe grobe Wurst, etwa 30 mm Pelle. Klassisch DACH-Region.

| Thickness | Doneness | Heat zone          | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ------------------ | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt zu indirekt | 230 °C     | 6 min    | 12 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Cervelat

Vorgegarter Schweizer Klassiker. Vor dem Grillen die Enden kreuzweise einschneiden ("Chrüsli"), optisch und für bessere Hitzeverteilung.

| Thickness | Doneness | Heat zone          | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ------------------ | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt zu indirekt | 250 °C     | 3 min 30 s | 7 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Ist vorgegart, Kerntemperatur egal, "durchgewärmt mit Farbe" ist das Ziel.
- Deckel offen lassen für bessere Char-Kontrolle.

### Riesencervelat

Wie Cervelat, nur grösser.

| Thickness | Doneness | Heat zone          | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ------------------ | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt zu indirekt | 220 °C     | 7 min    | 14 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Grillschnecke / Bratwurstschnecke

Aufgerollte rohe Wurst auf zwei Spiessen.

| Thickness | Doneness | Heat zone          | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ------------------ | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt zu indirekt | 220 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Die Spiesse im 90°-Winkel durchstechen, damit die Schnecke flach hält, sonst rollt sie sich auf.

### Kalbsbratwurst (St. Galler)

Vorgegart oder halbroh; Schweizer St. Galler Bratwurst wird NIE eingestochen, NIE aufgeschnitten.

| Thickness | Doneness | Heat zone          | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ------------------ | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt zu indirekt | 220 °C     | 4 min 30 s | 9 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Nicht einstechen, kein Senf (traditionelle Schule). Nicht aufschneiden.
- Hitze moderat halten: Pelle platzt über 230 °C schnell auf.

### Merguez

Dünn, scharf, Lamm und Rind. Schnell.

| Thickness | Doneness | Heat zone          | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ------------------ | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt zu indirekt | 220 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Italian sausage (Salsiccia)

Roh, etwa 25 mm Pelle.

| Type | Heat zone | Grate temp | Timing | Total | Turns | Rest |
| --- | --- | --- | --- | --- | --- | --- |
| Salsiccia ~100 g | Indirekt + direktes Finish | 180 zu 220 °C | 8 ind + 2 direkt | 10–12 min | Vierteldrehung alle 2 min | 2 min |

Sources: [Weber-US] Italian sausage 12–15 min. [SE] ähnlich.

### Chorizo (fresh)

| Type | Heat zone | Grate temp | Timing | Total | Turns |
| --- | --- | --- | --- | --- | --- |
| Chorizo fresco ~80 g | Indirekt + direktes Finish | 180 zu 220 °C | 8–10 ind + 2 direkt | 10–12 min | Vierteldrehung alle 2 min |

Sources: [Weber-US]. Cured Chorizo ist vorgegart, dann nur 4–6 min direkt mittel.

### Frankfurter / Wienerli

Vorgegart.

| Type                   | Heat zone     | Grate temp | Timing  | Total   | Turns                       |
| ---------------------- | ------------- | ---------- | ------- | ------- | --------------------------- |
| Wienerli / Frankfurter | Direkt mittel | 200 °C     | 4–6 min | 4–6 min | Vierteldrehung alle 1.5 min |

Sources: [Weber-DE].

### Nürnberger Bratwurst

Klein, etwa 20 g pro Stück, Schwein, viel Majoran.

| Type                | Heat zone     | Grate temp | Timing  | Total   | Turns                     |
| ------------------- | ------------- | ---------- | ------- | ------- | ------------------------- |
| Nürnberger ~20–25 g | Direkt mittel | 200 °C     | 4–6 min | 4–6 min | Vierteldrehung alle 1 min |

Sources: [Weber-DE] Nürnberger 4–5 min. [Grillportal] gleich.

---

## Various meats (Diverse Fleischsorten)

### BBQ-Speck (Speckscheiben vom Grill)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 7 min 30 s | 15 min | Einmal halbzeitig wenden | 5 min |

Sources: [Migros-Grilltimer].

Notes:

- Speck auf direkter Flamme ergibt einen Fettbrand. Immer perforierte Alufolie oder eine Grillplatte verwenden.

### Hamburger (Hackfleisch-Patties)

Das Serious-Eats-Schema: Dicke plus Smash gegen Pub-Style. Alle Temperaturen sind Pull-Temperaturen.

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 4 min 30 s | 9 min | Einmal halbzeitig wenden | 3 min |

Sources: [Migros-Grilltimer].

Notes:

- Käse 30 s vor Schluss auflegen, Deckel zu zum Schmelzen.

### Mixed-Grill Spiessli (Gemischte Spiessli)

Die langsamste Komponente bestimmt die Zeit; alle Stücke gleich dick halten.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 3 min    | 6 min | Einmal halbzeitig wenden | 3 min |

Sources: [Migros-Grilltimer].

### Kaninchenfilets (Kaninchen-Filets)

Mager, delikat, sehr wenig Fett, gart schnell, übergart noch schneller.

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest  |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ----- |
| -         | -        | Direkt, Deckel zu | 230 °C     | 2 min 30 s | 5 min | Einmal halbzeitig wenden | 3 min |

Sources: [Migros-Grilltimer].

---

## Fish (Fisch)

Allgemeine Regeln:

- **Rost immer aggressiv vorheizen und ölen** oder Fischkorb / Zedernholzbrett verwenden.
- **Hautseite zuerst nach unten**, nicht wenden, bis die Kruste sich von selbst löst.
- Bei den meisten Fischen bei 50–55 °C ziehen (gart nach, das Eiweiss setzt sich um 60 °C).

### Lachsfilet (Salmon fillet, single portion, skin-on)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 5 min    | 10 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Haut vor dem Ölen mit Küchenpapier sehr trocken tupfen, nur so wird sie knusprig.
- In den ersten 3 min auf der Hautseite nicht bewegen, sie löst sich, wenn sie bereit ist.

### Lachs (Whole side of salmon)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 3 min    | 6 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Nicht wenden, von der Haut auf ein Servierbrett heben.
- Auf Holzbrett dauert es etwa 5 min länger.

### Forelle (Whole trout, ~300 g)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total      | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ---------- | ------------------------ | ---- |
| 1 cm      | -        | Direkt, Deckel zu | 200 °C     | 2 min 45 s | 5 min 30 s | Einmal halbzeitig wenden | -    |
| 2 cm      | -        | Direkt, Deckel zu | 200 °C     | 5 min      | 10 min     | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Fischkorb verwenden, Forellenhaut ist delikat und reisst schnell.
- Mit Zitrone und Kräutern füllen.

### Dorade / Branzino (Whole sea bream / sea bass, ~400 g)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 9 min    | 18 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Haut auf jeder Seite mit 3 diagonalen Schnitten einritzen, Hitze dringt ein, Haut wirft sich nicht.
- Bauchhöhle salzen, mit Zitronenscheiben und Kräutern füllen.

### Gelbflossenthunfisch (Yellowfin tuna steak)

Maximales Anbraten, innen rare oder höchstens medium, alles darüber ist Tunfischverschwendung.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 250 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Kanten 15 s auf dem Rost rollen für vollständige Krustendeckung.

### Schwertfisch (Swordfish steak)

| Thickness | Doneness     | Heat zone         | Grate temp | Per side | Total    | Turns         | Rest  |
| --------- | ------------ | ----------------- | ---------- | -------- | -------- | ------------- | ----- |
| 2 cm      | medium 55 °C | Direkt mittelhoch | 230 °C     | 3 min    | 6 min    | Einmal wenden | 2 min |
| 3 cm      | medium       | Direkt mittelhoch | 230 °C     | 4–5 min  | 8–10 min | Einmal wenden | 2 min |

Sources: [Weber-US] Schwertfisch 3 cm 8–10 min. [BBCGF] 6–8 min für 2 cm.

Notes:

### Riesencrevetten (King prawns, shell-on)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- "C"-Form ist perfekt. Geschlossenes "O" bedeutet übergart.
- 2 parallele Spiesse pro 3–4 Crevetten verwenden, damit sie sich nicht drehen.

### Kalmar (Squid / Calamari)

Sehr schnell oder sehr langsam, nichts dazwischen.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 230 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

---

## Vegetables / Vegi / Vegan

Allgemeine Regeln:

- Gemüse leicht ölen (nicht den Rost). NACH dem Grillen salzen, Salz zieht Wasser und dämpft das Gemüse.
- Direkt mittel (200 °C) ist die Basis. Sehr feuchte Sorten (Tomate, Zucchetti) vertragen kurz direkt hoch; stärkehaltige (Süsskartoffel, Mais) wollen länger oder indirekt.

### Aubergine (Eggplant slices)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 6 min    | 12 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Optional: salzen, 15 min abtropfen lassen, trocken tupfen (entfernt Bitterstoffe; moderne Auberginen brauchen das selten).

### Champignons (Mushrooms, white or brown, whole)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 3 min    | 6 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Nicht waschen, mit feuchtem Tuch abreiben; Wasser dämpft, statt zu rösten.

### Portobello (Cap, whole)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min 30 s | 5 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Lamellenseite in den letzten 2 min mit Käse oder Kräutern füllen.

### Cherrytomaten-Spiess

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 230 °C     | 2 min 30 s | 5 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Halloumi

Grillt sich gut, weil er nicht schmilzt; sehr proteinreich, sehr fehlerverzeihend.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Paneer

Weniger fehlerverzeihend als Halloumi, wird heiss schnell gummig.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 5 min    | 10 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Knoblauch (Garlic, whole head in foil)

| Thickness | Doneness | Heat zone         | Grate temp | Per side    | Total  | Turns               | Rest |
| --------- | -------- | ----------------- | ---------- | ----------- | ------ | ------------------- | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 12 min 30 s | 25 min | alle 2–3 min drehen | -    |

Sources: [Migros-Grilltimer].

### Maiskolben (Corn on the cob)

Zwei Methoden: enthülst direkt, oder im Blatt (gewässert) indirekt.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 5 min    | 10 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Pak Choi (halved)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 6 min    | 12 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Mit Öl und Sojasauce einpinseln; aufpassen, geht in 30 s von Char zu verbrannt.

### Peperoni (Bell peppers)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 7 min 30 s | 15 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Spargeln (Asparagus, green)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 3 min 30 s | 7 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Weisser Spargel: schälen, 3 min vorkochen, dann indirekt 8–10 min in Alufolie mit Butter grillen.

### Süsskartoffel (Sweet potato)

| Thickness | Doneness | Heat zone         | Grate temp | Per side    | Total  | Turns               | Rest |
| --------- | -------- | ----------------- | ---------- | ----------- | ------ | ------------------- | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 17 min 30 s | 35 min | alle 2–3 min drehen | -    |

Sources: [Migros-Grilltimer].

### Tofu (fest, gepresst)

20–30 min unter Gewicht pressen, Oberfläche trocknen.

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 220 °C     | 5 min    | 10 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Stark ölen, Tofu klebt sonst. Sauberen, heissen Rost verwenden.
- Nach dem Pressen marinieren, nicht davor.

### Tomate (ganz oder halbiert)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Zucchetti / Zucchini (slices lengthwise)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 3 min    | 6 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Fenchel (Fennel, halved or wedges)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 7 min 30 s | 15 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Frühlingszwiebeln (Spring onions, whole)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min 30 s | 5 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Plant-based burger patty (Beyond, Impossible, Cornatur)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 250 °C     | 4 min 30 s | 9 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- NICHT aggressiv vorab auftauen, direkt aus dem Kühlschrank grillen.
- Nicht andrücken, sie verlieren Fett und trocknen aus.
- Keine offizielle Kerntemperaturempfehlung, garen, bis sie durchgewärmt sind (65–70 °C Kern).

### Cornatur-style veggie steak (seitan / wheat-protein)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

---

## Fruit (Früchte)

Regeln für gegrilltes Obst: trockene Oberfläche, heisser Rost, leicht ölen, nicht bewegen. Hoher Zuckergehalt bedeutet schnellen Char, also aufpassen.

### Ananas (Pineapple, rings)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Apfel (Apple, halved or rings)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 4 min    | 8 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Aprikosen (Apricots, halved)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Avocado (Halved, pit removed)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Nur die Schnittseite bekommt Grillstreifen, nicht wenden.
- Muss perfekt reif sein, unreif schmeckt nach nichts, überreif zerfällt.

### Banane (Banana, in skin, slit)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total  | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ------ | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 7 min 30 s | 15 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Feigen (Figs, halved)

| Thickness | Doneness | Heat zone         | Grate temp | Per side   | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | ---------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min 30 s | 5 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Balsamico-Glasur und Honig in den letzten 30 s.

### Mango (Cheeks / wedges)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Marroni (Chestnuts, scored)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total  | Turns               | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ------ | ------------------- | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 15 min   | 30 min | alle 2–3 min drehen | -    |

Sources: [Migros-Grilltimer].

Notes:

- X-Schnitt ist zwingend, ohne Einschnitt explodieren Marroni.
- Warm schälen, kalt klebt die Schale.

### Nektarinen (Nectarines, halved)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Pfirsiche (Peaches, halved)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

### Wassermelone (Watermelon, thick slices)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Scheiben mit Küchenpapier sehr trocken tupfen, Wassermelone besteht meist aus Wasser und braucht einen HEISSEN Rost für Marken.

### Zitrone / Limette (Lemon / Lime, halved, for dressing)

| Thickness | Doneness | Heat zone         | Grate temp | Per side | Total | Turns                    | Rest |
| --------- | -------- | ----------------- | ---------- | -------- | ----- | ------------------------ | ---- |
| -         | -        | Direkt, Deckel zu | 200 °C     | 2 min    | 4 min | Einmal halbzeitig wenden | -    |

Sources: [Migros-Grilltimer].

Notes:

- Gibt nach dem Grillen etwa 50 % mehr Saft, der Zitruszucker karamellisiert für ein völlig anderes Aroma.

---

## Sources

- `[Weber-DE]`: Weber Grillakademie und Rezepte (DE). <https://www.weber.com/DE/de/rezepte/>
- `[Weber-US]`: Weber Rezepte und Grilling Guides (US). <https://www.weber.com/US/en/recipes/>
- `[SE]`: Serious Eats, Kenji López-Alt, Artikel zur Fleischwissenschaft. <https://www.seriouseats.com/grilling-5117957>
- `[Migros-Grilltimer]`: Migros Grilltimer / Grillitarier App. <https://grillitarier.migros.ch/>
- `[Meathead]`: AmazingRibs / Meathead Goldwyn. <https://amazingribs.com/>
- `[Napoleon]`: Napoleon Grills Rezeptbibliothek. <https://www.napoleon.com/en/ca/grills/recipes>
- `[Traeger]`: Traeger Grills Rezepte. <https://www.traeger.com/recipes>
- `[BBCGF]`: BBC Good Food, Grill- und BBQ-Guides. <https://www.bbcgoodfood.com/recipes/collection/barbecue-recipes>
- `[Grillportal]`: Grillportal.de (DACH-Community). <https://www.grillportal.de/>
- `[USDA]`: USDA Food Safety, sichere Kerntemperaturen. <https://www.fsis.usda.gov/food-safety/safe-food-handling-and-preparation/food-safety-basics/safe-temperature-chart>
- `[BLV-CH]`: Bundesamt für Lebensmittelsicherheit (Schweiz). <https://www.blv.admin.ch/>
- Beyond Meat Grilling Guide. <https://www.beyondmeat.com/en-US/products>
- `[Steven Raichlen]`: Project Smoke / BBQ Bible, zitiert für Ribs und geräucherte Cuts. <https://barbecuebible.com/>
- `[Heston Blumenthal]`: In Search of Perfection, Steak-Methodik (häufiges Wenden). <https://www.youtube.com/watch?v=AmC9SmCBUj4>

---

## Offene Fragen / Lücken

Punkte, an denen Quellen materiell auseinandergehen oder Grillmi eine Marco-Entscheidung braucht:

- **Pull-Temperatur für Schwein, 63 °C (USDA / modern) gegen 70 °C (konservativ DACH)**. `[Migros-Grilltimer]` und neuere Schweizer Quellen nutzen 62–63 °C für ganzes Schweinemuskelfleisch, ältere Schweizer Literatur nennt weiterhin 70 °C. Grillmi-Standard gewählt: **63 °C für ganzes Muskelfleisch, 70 °C für Hackfleisch und Braten**. Bestätigen, dass das für ein Schweiz-zuerst-Publikum passt, oder als benutzerseitige Voreinstellung anbieten.
- **Pull-Temperatur für selbstgemachte Burger-Patties**. USDA und BLV nennen 70 °C, weil das Mahlen das Innere kontaminiert. Ein grosser Teil der Schweizer BBQ-Kultur isst Burger trotzdem medium-rare. Grillmi-Standard gewählt: **70 °C Pull**, mit explizitem Override "Nur selbst gemahlen, auf eigenes Risiko". Braucht UX-Überlegung.
- **Einmal wenden gegen häufig wenden bei Steaks**. Beides funktioniert, die Forschung stützt häufiges Wenden als leicht gleichmässiger. Aktueller Grillmi-Standard: einmal wenden bis 3 cm, alle 60 s wenden bei Reverse-Sear und dicken Cuts. Liesse sich vereinfachen.
- **Senf-Debatte zur St. Galler Bratwurst**: reine Tradition, keine Lebensmittelsicherheits-Frage; falls Grillmi Tipps zeigt, brauchen wir eine Position. Vorschlag: neutralen Hinweis zeigen, ohne Empfehlung.
- **Pferdesteak**: nur Schweizer Quellen, keine Weber- oder Serious-Eats-Abdeckung. Werte basieren auf `[Migros-Grilltimer]` allein, nicht trianguliert. Als "Schweizer Spezialität, Einzelquelle" markieren, falls wir Quellenvertrauen anzeigen.
- **Riesencrevetten-Zeiten**: `[Migros-Grilltimer]` nennt 3 min/Seite, `[Weber-DE]` 2 min/Seite. 50 % Diskrepanz. 2 min/Seite gewählt (Weber passt besser zur Crevettengrösse), wer es fester mag, gibt 30 s dazu.
- **Ribs-Methode (3-2-1 gegen ungewickelt)**: massive BBQ-Forum-Debatte, keine einzelne richtige Antwort. Grillmi-Standard gewählt: **2-2-1 für Baby Back, 3-2-1 für St. Louis**, mit ungewickelter Alternative für Puristen.
- **Chicken Wings**: backpulver-trockenpökeln ist messbar knuspriger [Meathead, SE], aber ein Extra-Schritt. Soll Grillmi standardmässig die einfache Version zeigen und den Backpulver-Tipp dazustellen, oder die optimierte Methode als Standard? Vorschlag: einfacher Standard, Tipp verfügbar.
- **Ganzer Fisch ohne Korb**: setzt einen gut geölten, heissen, sauberen Rost voraus. In der Praxis zerreissen Nutzer:innen die Haut. Grillmi sollte für Forelle und Dorade einen Fischkorb empfehlen und das deutlich sagen.
- **Plant-based-Patties Kerntemperatur**: keine offizielle Empfehlung von Beyond oder Impossible. 65–70 °C "durchgewärmt" gewählt, in Analogie zur Hackfleisch-Sicherheit; diese Produkte enthalten aber kein rohes Protein, das pasteurisiert werden müsste, also überdimensioniert. Position bestätigen.
- **Dicker Spargel**: Zeiten für weissen Spargel sind in Grillquellen schlecht abgedeckt; die meisten Schweizer Quellen kochen und buttern. Falls Grillmi weisse Spargeln zeigt, ist die obige Vorkoch-und-Grill-Methode eine von mehreren plausiblen Varianten.
- **Abdeckung mariniert / fertig gewürzt**: Werte gelten für unmarinierte Cuts. Marinierte oder fertig gewürzte Handelsprodukte können je nach Zucker- und Salzgehalt schneller bräunen oder festere Krusten bilden; ggf. 30–60 s/Seite kürzen oder indirekt zu Ende garen.
- **Höhen- und Windkorrekturen**: keine der Quellen liefert einen sauberen Anpassungsfaktor. Im Schweizer Kontext (Alpen) verlängert windiges Grillen im Freien auf Höhe indirekte Garzeiten realistisch um 10–20 %. Out of scope für v1, aber als zukünftiger Punkt notiert.
