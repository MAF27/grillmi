...
6. The Account page lists all active sessions for the user with a parsed device label, IP, and last-active relative time, and lets the user revoke any session except the current one.
...
13. The login page accepts the `?next=` query string and, on success, navigates to that path instead of `/`. The activation link's `set-password` page does the same with `?next=` if present.
...
1. `users` (`id` UUID PK, `email` citext UNIQUE, `password_hash` text, `created_at` timestamptz, `last_login_at` timestamptz nullable, `is_archived` bool default false).
2. `sessions` (`token` varchar PK, `user_id` UUID FK ON DELETE CASCADE, `csrf_token` varchar, `created_at` timestamptz, `expires_at` timestamptz INDEXED, `ip_address` inet, `user_agent` text).
...
**Why structlog instead of stdlib logging.** Per `stack-research-backend-apr-2026.md` §8: contextvars-based binding (request_id and user_id flow into every log line inside a request without manual passing), processor pipelines (one config produces colorised dev output and JSON prod output), and orjson rendering for ~85k JSON lines per second vs stdlib's ~22k. The audit log writes go to both `audit_log` Postgres table (durable) and to structlog (queryable by `journalctl -u grillmi-api -p info | jq 'select(.audit==true)'`).
...
| Risk | Mitigation |
| ---- | ---------- |
| Argon2id verify burst on simultaneous logins fills the 4 GB LXC | Each verify runs in `asyncio.to_thread` and consumes 64 MiB transiently; the LXC bumps to 2 CPU and 4 GB before deploy (per `stack-research-backend-apr-2026.md` §12) which gives a 2x margin. Per-IP rate limit caps to 5 per minute. |
| Postgres install fails on the live prod LXC | Phase 11 deploys to dev first, runs the full E2E suite there, and only then schedules prod. Rollback path in the runbook restores the prior LXC snapshot from PBS and reverts the `app_grillmi` role to the pre-bump revision. |
...
- [ ] Create `backend/` at the repo root with the `src` layout: `backend/grillmi/{__init__.py, main.py, config.py, db.py, logging.py, security/, repos/, routes/, templates/emails/, cli/, email/, models/, sync/}` and `backend/tests/{conftest.py, unit/, integration/}`.
- [ ] Add a top-level `backend/README.md` documenting the `uv sync && uv run alembic upgrade head && uv run granian --interface asgi grillmi.main:app` dev-loop and a one-liner pointing at `260428-accounts-and-sync.md` for the full spec.
- [ ] Write `backend/pyproject.toml` declaring runtime deps `fastapi[standard]>=0.136,<0.137`, `granian>=2.0,<3`, `uvicorn>=0.34`, `asyncpg>=0.31,<0.32`, `sqlalchemy[asyncio]>=2.0.49,<2.1`, `alembic>=1.18,<2`, `pydantic-settings>=2.14,<3`, `structlog>=25,<26`, `orjson>=3.10`, `argon2-cffi>=23.1`, `aiosmtplib`, `httpx>=0.28`, `jinja2`. Dev deps: `pytest>=8.3`, `pytest-asyncio>=0.25`, `pytest-mock`, `pytest-cov`, `testcontainers[postgres]>=4.8`. Set `[tool.pytest.ini_options] asyncio_mode = "auto"` and `[tool.coverage.report] fail_under = 80`.
- [ ] Implement `backend/grillmi/config.py` using `pydantic_settings.BaseSettings` with required fields `APP_ENV`, `PUBLIC_BASE_URL`, `DATABASE_HOST`, `DATABASE_PORT`, `DATABASE_NAME`, `DATABASE_USER`, `DATABASE_PASSWORD`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_FROM_ADDRESS`, `HOSTPOINT_SMTP_USER`, `HOSTPOINT_SMTP_PASSWORD`, `HIBP_USER_AGENT`, `SESSION_MAX_AGE_HOURS`, `SESSION_COOKIE_NAME`, `RATE_LIMIT_LOGIN_IP_PER_MIN`, `RATE_LIMIT_LOGIN_ACCOUNT_PER_HOUR`, `AUDIT_RETENTION_DAYS`, `TOMBSTONE_RETENTION_DAYS`, `OPENAPI_ENABLED`. Construct `DATABASE_URL` as `postgresql+asyncpg://{DATABASE_USER}:{DATABASE_PASSWORD}@{DATABASE_HOST}:{DATABASE_PORT}/{DATABASE_NAME}`.
- [ ] Implement `backend/grillmi/db.py` exposing `engine`, `async_session_maker`, and a `get_session()` FastAPI dependency that yields `AsyncSession` and rolls back on exception.
- [ ] Implement `backend/grillmi/main.py` with the FastAPI app factory: when `settings.OPENAPI_ENABLED` is true, instantiate `FastAPI(docs_url="/docs", redoc_url="/redoc")`; otherwise `FastAPI(docs_url=None, redoc_url=None)`. Wire startup and shutdown lifespan tasks (hourly session cleanup; structlog initialisation).
- [ ] Implement `backend/grillmi/middleware.py` registering the security-headers middleware, the request-id binder (uuid per request, bound to structlog contextvars), and the CSRF middleware that runs after auth and before route handlers.
...
- [ ] Implement `backend/grillmi/logging.py` configuring structlog with the JSON renderer, contextvars binding, and a `ProcessorFormatter` that captures Granian access logs into the same pipeline.

**Phase 2: Postgres schema and initial Alembic migration**

- [ ] `cd backend && alembic init -t async migrations` then edit `migrations/env.py` to import `backend.grillmi.db.Base` and to read the URL from `Settings().DATABASE_URL`.
- [ ] Define SQLAlchemy ORM models in `backend/grillmi/models/` (one file per table: `user.py`, `session.py`, `password_reset_token.py`, `audit_log.py`, `grillade.py`, `grillade_item.py`, `menu.py`, `menu_item.py`, `favorite.py`, `settings.py`). Use typed `Mapped[...]` declarative; primary keys default to `uuid.uuid4()` client-side.
...
- [ ] Add a hand-written second revision `0002_seed_extensions.py` only if `0001_initial.py` cannot enable the `citext` extension cleanly inside autogenerate; otherwise keep one initial revision.

**Phase 3: Auth standard primitives**

- [ ] Implement `backend/grillmi/security/argon2.py` exposing `hash_password()`, `verify_password()`, and a module-load `_DUMMY_HASH = _hasher.hash("dummy-password-for-timing-safety")`. Both `hash` and `verify` wrap `asyncio.to_thread(...)`. Parameters: `memory_cost=65536, time_cost=3, parallelism=4, hash_len=32, salt_len=16`.
- [ ] Implement `backend/grillmi/repos/sessions_repo.py` with `create_session(user_id) -> (token, csrf_token, expires_at)`, `get_session(token)`, `delete_session(token)`, `delete_sessions_for_user(user_id)`, `cleanup_expired()`. Token = `secrets.token_urlsafe(32)`.
- [ ] Implement `backend/grillmi/security/csrf.py` with `validate_csrf(request, session) -> None | raises HTTPException(403)`, comparing the `X-CSRFToken` header to `session.csrf_token` via `secrets.compare_digest`. Exempt GET/HEAD/OPTIONS.
- [ ] Implement `backend/grillmi/security/rate_limit.py` with two sliding-window in-memory counters: `login_ip_limiter` (5 per 60s, key = client IP) and `login_account_limiter` (10 per 3600s, key = email). Public surface: `check_and_record(key) -> None | raises HTTPException(429)`.
- [ ] Implement `backend/grillmi/security/headers.py` middleware that sets `X-Frame-Options DENY`, `X-Content-Type-Options nosniff`, `Referrer-Policy strict-origin-when-cross-origin`, `Strict-Transport-Security max-age=31536000; includeSubDomains` (HTTPS only), and a strict `Content-Security-Policy default-src 'self'; frame-ancestors 'none'`.
- [ ] Implement `backend/grillmi/repos/audit_log_repo.py` with `record(action, user_id, email, ip, success)` that inserts a row and emits a structlog INFO line with `audit=True`. Wrap calls in `asyncio.create_task` so they never block the request.
- [ ] Implement `backend/grillmi/security/hibp.py` with `check_password(password) -> bool` performing the SHA-1 prefix range API call against `https://api.pwnedpasswords.com/range/{prefix}` using `httpx.AsyncClient` with a 5-second timeout and the `HIBP_USER_AGENT` header. On API failure, log a warning and return `False` (fail-open, password allowed).
- [ ] Implement `backend/grillmi/email/sender.py` exposing `send(to, subject, body_text)` via `aiosmtplib` against `SMTP_HOST:SMTP_PORT` with STARTTLS, authenticating with `HOSTPOINT_SMTP_USER` and `HOSTPOINT_SMTP_PASSWORD`, From = `SMTP_FROM_ADDRESS`. Add `backend/templates/emails/activation.de.txt` and `backend/templates/emails/password-reset.de.txt` (Jinja2; receive `recipient_email`, `link`, `expires_hours`).
- [ ] Activation email body (German, plain text): subject `Grillmi: Konto aktivieren`. Body explains the link is valid for 72 hours, identifies the app, and is signed off `Bis bald am Grill, Grillmi`. Reset email body: subject `Grillmi: Passwort zurücksetzen`, identical structure but 30-minute expiry text, plus a short line about ignoring the email if the user did not request it.
- [ ] Implement `backend/grillmi/email/templates.py` with `render_activation(link, expires_hours, recipient)` and `render_reset(link, expires_hours, recipient)` returning `(subject, body_text)`. Loads templates via Jinja2's `FileSystemLoader('backend/templates/emails')`. Used by both auth routes and the admin CLI.

**Phase 4: Auth routes and admin password-reset CLI**

- [ ] Implement `backend/grillmi/routes/auth.py` with `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me`, `POST /api/auth/forgot-password`, `POST /api/auth/set-password`, `GET /api/auth/sessions`, `POST /api/auth/sessions/{id}/revoke`, `DELETE /api/auth/account`. Each route follows the exact sequence in `~/dev/reference/auth.md` §Login Flow and §Invitation & Password Reset Flow. Token pages call logout on load (clear-session-on-token-pages); activation auto-logs-in, reset does not.
- [ ] Implement `backend/grillmi/cli/admin_init.py` exposing `grillmi admin-init --email <email>` (entry point declared in `pyproject.toml`'s `[project.scripts]`). Behaviour: idempotent insert into `users` with `password_hash = "!disabled_" + secrets.token_hex(8)`, generate an invitation token with `secrets.token_urlsafe(32)`, store `sha256(token)` in `password_reset_tokens` (kind=`invitation`, expires +72h), send the activation email. Skip silently if the email already has a user row.
- [ ] Implement `backend/grillmi/cli/admin_reset.py` exposing `grillmi admin-reset` with the `read -s` plus temp-file plus pipe pattern from `auth.md` §Admin Password Reset Script. Hashes via `argon2.hash_password()`, calls `delete_sessions_for_user(user_id)` after writing the new hash, prints only success or failure.
...
- [ ] Implement `backend/grillmi/repos/grilladen_repo.py`, `grillade_items_repo.py`, `menus_repo.py`, `menu_items_repo.py`, `favorites_repo.py`, `settings_repo.py`. Every public function takes `session: AsyncSession, user_id: UUID` as the first two args. Queries always filter on `user_id`. Soft-delete sets `deleted_at = func.now()` and writes `updated_at = func.now()`.
- [ ] Implement `backend/grillmi/routes/grilladen.py` with `GET /api/grilladen?since=`, `POST /api/grilladen`, `GET /api/grilladen/{id}`, `PATCH /api/grilladen/{id}`, `DELETE /api/grilladen/{id}`, `GET /api/grilladen/{id}/items?since=`, `POST /api/grilladen/{id}/items`, `PATCH /api/grilladen/{id}/items/{item_id}`, `DELETE /api/grilladen/{id}/items/{item_id}`. PATCH compares request `updated_at` to persisted `updated_at` and returns 409 if request is older. Stranded child items return 409.
- [ ] Implement `backend/grillmi/routes/menus.py` mirroring the Grilladen route shape for `/api/menus` and `/api/menus/{id}/items`.
- [ ] Implement `backend/grillmi/routes/favorites.py` for `GET/POST /api/favorites`, `GET/PATCH/DELETE /api/favorites/{id}`, with the same `?since=` and LWW semantics.
- [ ] Implement `backend/grillmi/routes/settings.py` for `GET /api/settings` and `PUT /api/settings` (replace-on-write of the jsonb blob).
- [ ] Implement `backend/grillmi/routes/sync.py` with `POST /api/sync/import`, accepting `{grilladen, menus, favorites, settings}` in one transaction, attributing to `request.state.user_id`, skipping rows whose `id` exists for that user.

**Phase 5b: User-scope test scaffolding**

- [ ] Add a session-scope fixture `two_users` in `backend/tests/integration/conftest.py` that creates user A and user B with distinct emails, returns `(user_a, user_b)`. Used by every isolation test.
- [ ] Add a small helper `assert_isolated(repo_func, user_a, user_b, sample_row)` in `backend/tests/integration/_isolation.py` that writes `sample_row` for user A, calls `repo_func` as user B, asserts no rows visible. Each isolation test is a one-liner that reuses this helper.

**Phase 6: Health, OpenAPI, logging**

- [ ] Implement `backend/grillmi/routes/health.py` with `GET /api/health`. Returns 200 `{status:"ok", db:true, smtp_reachable:true}` when healthy. DB probe is `SELECT 1`; SMTP probe is a synchronous TCP `socket.create_connection((SMTP_HOST, SMTP_PORT), timeout=2)` wrapped in `asyncio.to_thread`. Returns 503 with `{status:"degraded", ...}` when DB unreachable.
- [ ] Wire conditional OpenAPI in `backend/grillmi/main.py` per Approach.
- [ ] Wire structlog initialisation in `backend/grillmi/main.py`'s lifespan startup; the audit logger is bound `logger="audit"`.

**Phase 7: Frontend auth store and routes**

- [ ] Implement `src/lib/api/client.ts` exporting `apiFetch(path, init)` with the contract described in Approach (auto CSRF header, auto-redirect on 401, refetch-on-409, queue-on-5xx).
- [ ] Implement `src/lib/stores/authStore.svelte.ts` matching the rune pattern in `favoritesStore.svelte.ts`. Public API: `get user`, `get csrfToken`, `init(serverState)`, `setSession({user, csrfToken})`, `clear()`.
- [ ] Update `src/routes/+layout.ts` to fetch `GET /api/auth/me`. On 200, return `{user, csrfToken}`. On 401, throw `redirect(303, '/login?next=' + encodeURIComponent(url.pathname))`. Skip the redirect if `url.pathname` is already `/login`, `/set-password`, or `/forgot-password`.
- [ ] Update `src/routes/+layout.svelte` to call `authStore.init(data)` on mount.
- [ ] Implement `src/routes/login/+page.svelte` with email and password fields, submit calls `POST /api/auth/login` (CSRF header populated from a meta tag rendered by `+layout.svelte` after first-load), on success populates `authStore` and navigates to `?next` or `/`.
- [ ] Implement `src/routes/set-password/+page.svelte` reading `?token=` from the URL. On mount, call `POST /api/auth/logout` if a session cookie is present (per auth.md clear-session-on-token-pages). Submit calls `POST /api/auth/set-password` with `{token, password}`. On invitation success, populate `authStore` from the response and navigate to `/`. On reset success, navigate to `/login`.
- [ ] Implement `src/routes/forgot-password/+page.svelte` with an email field; submit calls `POST /api/auth/forgot-password` and shows the always-the-same German confirmation toast.
- [ ] Implement `src/routes/account/+page.svelte` showing email read-only, a "Passwort ändern" button that calls `POST /api/auth/forgot-password` for the current email, the parsed session list from `GET /api/auth/sessions` with per-row "Abmelden" button calling `POST /api/auth/sessions/{id}/revoke`, and a "Konto löschen" `HoldButton` (500ms) that calls `DELETE /api/auth/account`, then `resetAll()`, then navigates to `/login`.

**Phase 8: IDB v3 to v4 migration**

- [ ] Update `src/lib/stores/db.ts`: bump `DB_VERSION` to 4. In the upgrade callback when `oldVersion < 4`, create a new `grilladen` object store (keyed by UUID) and a new `syncQueue` store (keyed by auto-increment). Migrate the singleton `sessions` (key `current`) and the singleton `planState` (key `current`) into a single `grilladen` row using `uuid()`; preserve `targetEpoch`, items, and status. Drop the legacy `sessions` and `planState` stores at the end of the callback.
- [ ] Add `listGrilladen()`, `putGrillade()`, `deleteGrillade()`, `listSyncQueue()`, `enqueueSync()`, `popSync()` to `db.ts` matching the existing helpers' shape.
- [ ] Extend `resetAll()` to clear the new `grilladen` and `syncQueue` stores.

**Phase 9: Sync queue**

- [ ] Implement `src/lib/sync/queue.ts` with the public surface `enqueue(write)`, `flush()`, and `attach()`. The `write` shape is `{entity, method, path, body, idbKey}`. `enqueue` writes a row to IDB store `syncQueue` with an auto-incremented sequence id; `flush` drains the queue in id order, calling `apiFetch` per row, removing the row only on 2xx. The 200ms debounce is a single shared `setTimeout` reset on every `enqueue`; the timeout fires `flush()`. `attach()` adds the `visibilitychange` listener and a `online` listener that triggers `flush()` immediately on reconnect.
- [ ] Implement the per-entity send mapping inside `src/lib/sync/queue.ts`: a small `dispatch(write)` helper resolves `{entity: 'grilladen', method: 'POST', body}` into `apiFetch('/api/grilladen', {method: 'POST', body: JSON.stringify(body)})`. Centralise the mapping so future entities are one entry.
- [ ] Implement `src/lib/sync/pull.ts` with `pull(since)` that calls every GET endpoint with the `?since=` watermark stored in IDB store `syncMeta`; merges results into IDB stores using the same LWW rule (server `updated_at` wins). Called on app foreground (`document.visibilitychange` to `visible`) and on Grillade open.
- [ ] Implement an IDB store `syncMeta` (key `lastPullEpoch`) maintained by `pull.ts`. On every successful pull, write `lastPullEpoch = Date.now() - 1000` (one-second skew protection).
- [ ] Wire `attach()` into `src/routes/+layout.svelte`'s `onMount`. Also call `pull(lastPullEpoch)` once on mount so a freshly-foregrounded tab gets a delta even if it never went hidden.

**Phase 10: First-login data migration**

- [ ] In `src/routes/set-password/+page.svelte`'s success handler for invitation activation, before navigating to `/`, read `listFavorites()`, `listSavedPlans()`, `getSettings()`, `getCurrentPlanState()`, `getCurrentSession()`, construct the `/api/sync/import` payload, POST it. On 200, write `firstLoginMigrationComplete: true` into IDB store `syncMeta`. On non-200, enqueue the same payload via the standard sync queue and proceed.
- [ ] Add a guard in `src/lib/sync/queue.ts` that no-ops if `firstLoginMigrationComplete` is false (so the regular queue does not double-import legacy rows).

**Phase 10b: Wire existing stores through the sync queue**

- [ ] Update `src/lib/stores/favoritesStore.svelte.ts` `save`, `rename`, `remove`, and `touch` actions to call `enqueueSync({entity: 'favorites', method, payload})` after the IDB write completes. The store keeps its current optimistic-update behaviour; the queue handles the network.
- [ ] Update `src/lib/stores/menusStore.svelte.ts` actions identically with `entity: 'menus'`.
- [ ] Update `src/lib/stores/settingsStore.svelte.ts` `setTheme`, `setSound`, `setVibrate`, `markFirstRunSeen` to call `enqueueSync({entity: 'settings', method: 'PUT', payload})` after each persist.
- [ ] Update `src/lib/stores/sessionStore.svelte.ts` to translate the existing single-session model into a Grillade UUID write at every state-change persist call. Preserve the local-only Wake-Lock behaviour exactly.

**Phase 11: Management script and Ansible role updates**

- [ ] Extend `/opt/grillmi/resources/scripts/grillmi`: add `migrate` (`cd /opt/grillmi/backend && doppler run -- alembic upgrade head`); extend `status` to include `grillmi-api.service` in the systemctl list and to GET `/api/health` for the HTTP probe; map `/api/health` `status` field onto the §5 `ok|degraded|down` exit-code contract from `service-management-scripts.md`. Add `backup [bucket]` and `restore <file>` per the runbook spec, with buckets `daily|weekly|monthly|manual`.
- [ ] Update the `app_grillmi` Ansible role per `260428-accounts-and-sync.md` (infra side file): LXC resize, Postgres 17 install, role and DB creation, `/etc/grillmi/config.env` template, FastAPI Granian systemd unit, Caddy `/api` reverse-proxy block, sudoers extension, backup timer and tombstone-GC timer, the `admin-init --email marco.fruh@me.com` task gated `when: grillmi_environment == "prod"` and `failed_when: false`.
- [ ] Update `vite.config.ts` to add `server.proxy: {'/api': {target: 'http://localhost:8000', changeOrigin: false, secure: false}}`.
...
- [ ] Push `feature/accounts-and-sync` to GitHub.
- [ ] Run `ansible-playbook playbooks/applications/grillmi-deploy.yml --limit grillmi_dev` and follow the runbook's dev verification steps end-to-end. See `260428-accounts-and-sync.md` (runbook) section "Staging checks (run on grillmi-dev)".
- [ ] Resize the prod LXC (atlas, CTID 114) to 2 CPU / 4 GB / 32 GB per `260428-accounts-and-sync.md` (infra) section "LXC resize".
- [ ] Run `ansible-playbook playbooks/applications/grillmi-deploy.yml --limit grillmi_prod` and follow the runbook's prod verification.
- [ ] Add the `grillmi` Vigil check on nexus per the infra side file.
...
1. `backend/tests/unit/test_argon2.py::test_hash_then_verify_succeeds`: a hashed password verifies; an empty password verifies false.
2. `backend/tests/unit/test_argon2.py::test_dummy_hash_verify_returns_false_with_unknown_email`: `verify_password_timing_safe(password, None)` runs the dummy-hash path and returns False.
3. `backend/tests/unit/test_argon2.py::test_check_needs_rehash_after_param_change`: changing parameters causes `check_needs_rehash` to return True.
4. `backend/tests/unit/test_csrf.py::test_csrf_passes_when_header_matches_session_token`: `validate_csrf` returns None.
5. `backend/tests/unit/test_csrf.py::test_csrf_rejects_when_header_missing`: returns 403.
6. `backend/tests/unit/test_csrf.py::test_csrf_exempts_get_head_options`: GET requests pass without the header.
7. `backend/tests/unit/test_rate_limit.py::test_ip_limiter_blocks_after_5_in_60s`: 6th call raises 429.
8. `backend/tests/unit/test_rate_limit.py::test_account_limiter_blocks_after_10_in_3600s`: 11th call raises 429.
9. `backend/tests/unit/test_rate_limit.py::test_window_slides_correctly`: after 61s the counter resets.
10. `backend/tests/unit/test_hibp.py::test_known_breached_password_is_flagged` (mocked httpx): SHA-1 prefix request returns the suffix and the function returns True.
11. `backend/tests/unit/test_hibp.py::test_api_failure_fails_open`: httpx raises, function returns False, warning is logged.
12. `backend/tests/unit/test_email_sender.py::test_send_calls_aiosmtplib_with_starttls` (mocked aiosmtplib): asserts STARTTLS, From, To, Subject.
13. `backend/tests/unit/test_logging.py::test_audit_event_writes_structlog_with_audit_true`: logs include `audit=True`.
14. `tests/unit/authStore.test.ts::test_set_session_populates_user_and_csrf_token`: store reflects the values.
15. `tests/unit/authStore.test.ts::test_clear_resets_to_null`: `authStore.user` is null after clear.
16. `tests/unit/syncQueue.test.ts::test_enqueue_persists_to_idb`: `enqueue` writes to `syncQueue` store.
17. `tests/unit/syncQueue.test.ts::test_flush_drains_queue_in_order`: replays POSTs in IDB-insertion order.
18. `tests/unit/syncQueue.test.ts::test_401_during_flush_persists_queue_and_redirects`: queue remains, `authStore.user` is null, redirect was called.
19. `tests/unit/syncQueue.test.ts::test_409_continues_queue`: a 409 on row N still allows row N+1 to flush.
20. `tests/unit/db.test.ts::test_v3_to_v4_migration_folds_planstate_and_session_into_grilladen`: a fixture v3 DB ends up with one row in `grilladen`.
21. `tests/unit/db.test.ts::test_resetAll_clears_grilladen_and_syncQueue`: both stores empty after `resetAll()`.
...
1. `test_users_repo_isolation.py::test_user_b_cannot_read_user_a_grilladen`: creates two users, writes one Grillade as A, asserts B's `list_for_user` is empty.
2. `test_grilladen_repo_isolation.py::test_user_b_cannot_patch_user_a_grillade`: PATCH as B raises 404 (not 403, to avoid existence leak).
3. `test_grillade_items_repo_isolation.py::test_user_b_cannot_delete_user_a_item`: DELETE as B raises 404.
4. `test_menus_repo_isolation.py::test_user_b_cannot_read_user_a_menus`: same shape.
5. `test_menu_items_repo_isolation.py::test_user_b_cannot_patch_user_a_menu_item`: same shape.
6. `test_favorites_repo_isolation.py::test_user_b_cannot_list_user_a_favorites`: same shape.
7. `test_settings_repo_isolation.py::test_user_b_settings_are_independent`: writing as B does not affect A's row.
8. `test_auth_login.py::test_login_with_correct_password_returns_session_cookie_and_csrf_token`: cookie is set, body contains `csrfToken`.
9. `test_auth_login.py::test_login_with_wrong_password_returns_generic_error`: response body says "Invalid email or password" and is identical for missing email.
10. `test_auth_login.py::test_login_response_time_identical_for_existing_and_missing_email`: average over 20 runs differs by less than 50ms.
11. `test_auth_login.py::test_login_rate_limit_per_ip_returns_429_after_5_attempts`: 6th attempt raises 429.
12. `test_auth_login.py::test_login_disabled_user_returns_generic_error`: a `!disabled_*` hash returns the same error as wrong password.
13. `test_auth_set_password.py::test_invitation_token_set_password_auto_logs_in`: response carries a session cookie.
14. `test_auth_set_password.py::test_reset_token_set_password_does_not_auto_log_in`: no session cookie in response.
15. `test_auth_set_password.py::test_set_password_invalidates_all_existing_sessions`: previously created sessions for the user are gone.
16. `test_auth_set_password.py::test_set_password_rejects_hibp_match` (mocked HIBP): returns 422.
17. `test_auth_set_password.py::test_token_pages_clear_existing_session_on_load`: a logout call lands before the page render.
18. `test_auth_forgot_password.py::test_response_is_identical_for_existing_and_missing_email`: same body, same status.
19. `test_auth_forgot_password.py::test_unactivated_user_gets_invitation_email_with_72h_expiry`: edge case from auth.md.
20. `test_auth_sessions.py::test_get_sessions_returns_only_callers_sessions`: scoped by user_id.
21. `test_auth_sessions.py::test_revoke_session_removes_row`: row gone, subsequent request with that token returns 401.
22. `test_auth_account_delete.py::test_delete_account_cascades_grilladen_menus_favorites_settings_sessions`: every owned row is gone.
23. `test_grilladen_routes.py::test_get_with_since_returns_only_newer_rows`: rows with older `updated_at` are excluded.
24. `test_grilladen_routes.py::test_patch_with_older_updated_at_returns_409`: LWW.
25. `test_grilladen_routes.py::test_one_active_grillade_per_user_partial_index_enforced`: second concurrent active row raises a constraint error.
26. `test_grillade_items_routes.py::test_stranded_item_returns_409`: parent missing, server returns 409.
27. `test_grillade_items_routes.py::test_delete_grillade_cascades_items`: all items gone.
28. `test_sync_import.py::test_import_attributes_to_caller_user_id`: rows have the right `user_id`.
29. `test_sync_import.py::test_import_is_idempotent_on_duplicate_ids`: re-running the same payload does not duplicate rows.
30. `test_admin_cli.py::test_admin_init_idempotent_on_existing_email`: second run is a silent no-op.
31. `test_admin_cli.py::test_admin_reset_invalidates_all_sessions`: sessions for the user are deleted.
32. `test_health.py::test_health_returns_200_when_db_and_smtp_reachable`: 200 with all true.
33. `test_health.py::test_health_returns_503_when_db_unreachable`: with the engine pointed at a closed port.
34. `test_security_headers.py::test_response_carries_required_headers`: every header from auth.md §Security Headers is present.

### E2E Tests

Located under `tests/e2e/` (Playwright). Run against the dev backend; backend is started via the management script's `start` verb.

1. `tests/e2e/auth.spec.ts::test_full_activation_flow`: admin-init from CLI, fetch the email body via a test SMTP capture fixture, click the link in a Playwright context, set a password, land logged-in on Home.
2. `tests/e2e/auth.spec.ts::test_forgot_password_then_reset_then_login`: full reset flow ends at `/login`, not auto-logged-in.
3. `tests/e2e/auth.spec.ts::test_login_then_logout_clears_idb`: after logout, `indexedDB.databases()` shows no Grillmi data.
4. `tests/e2e/sync.spec.ts::test_grillade_created_in_context_a_appears_in_context_b`: two browser contexts, same user; create in A, foreground in B, see the row.
5. `tests/e2e/sync.spec.ts::test_concurrent_patch_resolves_lww`: A and B PATCH the same row; older write returns 409, newer wins.
6. `tests/e2e/sync.spec.ts::test_offline_writes_replay_on_reconnect`: go offline, edit two items, go online, server reflects both.
7. `tests/e2e/account.spec.ts::test_account_page_shows_current_session_marker`: current session row has the marker.
8. `tests/e2e/account.spec.ts::test_revoke_other_session_logs_that_browser_out`: second context is logged out within 10s.
9. `tests/e2e/account.spec.ts::test_password_change_button_sends_reset_email`: hits forgot-password endpoint, toast appears.
10. `tests/e2e/account.spec.ts::test_account_delete_with_hold_button_redirects_to_login_and_wipes_idb`: 500ms hold, 204 from server, IDB empty.
11. `tests/e2e/account.spec.ts::test_session_list_renders_parsed_device_label`: a Mac Safari context shows "Mac, Safari" or equivalent in the device-label column.
12. `tests/e2e/sync.spec.ts::test_first_login_bulk_import_runs_once`: seeds IDB with five favorites, completes activation, verifies the server has those five rows, second login does not re-import.
13. `tests/e2e/sync.spec.ts::test_pull_after_foreground_fetches_recent_changes`: device A creates a Grillade while device B is hidden; foreground B and confirm the row appears via the foreground pull, not via a write coming back.
14. `tests/e2e/auth.spec.ts::test_set_password_page_calls_logout_on_load_when_cookie_exists`: a logged-in user opening a fresh activation link is signed out before the form renders.
15. `tests/e2e/auth.spec.ts::test_login_after_24h_expiry_returns_to_original_url`: forge a 25-hour-old cookie, navigate to `/account`, confirm bounce to `/login?next=%2Faccount` and that re-login lands back on `/account`.
16. `tests/e2e/auth.spec.ts::test_rate_limit_per_ip_returns_429_after_5_attempts`: six rapid wrong-password POSTs from one context produce a 429 with `Retry-After`.
17. `tests/e2e/auth.spec.ts::test_security_headers_present_on_html_response`: `GET /` includes `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Strict-Transport-Security`, and a `Content-Security-Policy`.
18. `tests/e2e/sync.spec.ts::test_grillade_soft_delete_propagates_to_other_device`: delete on A, foreground B, the row is gone from B's UI within five seconds.
19. `tests/e2e/account.spec.ts::test_revoking_current_session_logs_caller_out`: revoking the row marked `is_current` redirects to `/login`.
20. `tests/e2e/sync.spec.ts::test_409_on_stale_patch_triggers_refetch`: client patches with stale `updated_at`, server responds 409, client refetches and the IDB row reflects the server state.
21. `tests/e2e/sync.spec.ts::test_offline_indicator_does_not_block_writes`: airplane mode on, edits land in IDB and queue, online again, edits flush.
22. `tests/e2e/auth.spec.ts::test_health_endpoint_is_unauthenticated`: `GET /api/health` succeeds with no cookie.
23. `tests/e2e/auth.spec.ts::test_openapi_disabled_in_prod`: `GET /docs` returns 404 when `OPENAPI_ENABLED=false`.

### Manual Verification (Marco)

1. On the iPhone after a fresh install, launch the PWA. The login screen appears; no IDB content is visible. Receive the activation email at `marco.fruh@me.com`, tap the link, set a password, and land on Home with all prior local Grilladen, Menüs, Favoriten, and Settings still present.
2. On the Mac in Safari, sign in with the same email. Create a new Grillade. On the iPhone, foreground the app. The new Grillade appears within five seconds.
3. On the iPhone, edit the Grillade name. On the Mac, foreground. The new name appears.
4. Open `/account` on the Mac. The session list shows two entries with parsed device labels (Mac, iPhone), IPs, and last-active relative times. Tap "Abmelden" on the iPhone session; confirm the iPhone is bounced to `/login` within ten seconds.
5. On the iPhone, request a password change from `/account`. The reset email arrives within thirty seconds. Tap the link, set a new password, land on `/login`. Sign in with the new password.
6. Toggle the Mac to airplane mode, edit two items in a Grillade, toggle airplane mode off. Inspect the iPhone shortly after; both edits are present.
7. From `/account` on the Mac, hold "Konto löschen" for 500ms. Confirm the redirect to `/login` and the empty IDB. Re-run `grillmi admin-init --email marco.fruh@me.com` on the prod LXC to seed the account again.
