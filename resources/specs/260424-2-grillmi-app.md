...
- **Pouletspiessli had two near-identical timing rows** (2.5 cm Würfel and 3 cm Würfel both topped at 12 min). The 0.5 cm split did not change the schedule and the two-row picker was just noise. Collapsed to a single row, `3 cm Würfel`, 10 to 12 min, `Direkt mittel`, rotate-flip. Source: `resources/docs/grill-timings-reference.md` §Pouletspiessli.
...
- **Migusto credit removed from Settings.** The Über-Grillmi panel now reads "Garzeiten basieren auf Migros Grilltimer, Weber, Serious Eats und Meathead." The single Migusto-tagged tip in the generated JSON (Cervelat note) was rewritten in German without the brand attribution.
- **Dev-doc sanitization (260426).** Top-level docs (`README.md`, `CLAUDE.md`), the design brief, and the UI architecture were reframed to stand on Grillmi's own merits without reverse-engineering language. In `resources/docs/grill-timings-reference.md`, mechanical brand tags were stripped from turn-column cells and the boilerplate extraction provenance was removed from `Sources:` lines, leaving short bracketed citations (`Sources: [Migros-Grilltimer].`) consistent with the Settings credit. Body sentences and methodology bullets that described what Migros does or named specific Migros SKUs were reworded; `[Migros-Grilltimer]` citation form is preserved alongside the bibliography entry. The two raw extraction files (`migros-grilltimer-dump.md` / `.json`) moved to a new gitignored `resources/docs/_private/` and will be expunged from git history in a follow-up filter-repo pass. The shipped `src/lib/data/timings.generated.json` is unchanged apart from `generatedAt`.
...
- **Favorit and Plan-Vorlage split.** Shipped per `resources/specs/260425-favorit-plan-vorlage-split.md`. `Favorite` now models a single configured grillable (cut + thickness/prep + Garstufe + cooking parameters) and lives in the `favorites` IDB store; the previous saved-plan list type was renamed to `SavedPlan` and moved into a new `plans` store. The IDB upgrade callback bumps version 1 → 2 and silently migrates the v1 `favorites` records into `plans` so existing users keep their saved plans intact. The `/favorites` route was renamed to `/plans` ("Plan-Vorlagen"), and the AddItemSheet first step gained a Favoriten tab plus an inline "Als Favorit speichern" action on the specs step. The Plan screen's "Als Favorit speichern" became "Plan speichern" and the inline `★ Favorit` loader became `★ Plan-Vorlage`.
