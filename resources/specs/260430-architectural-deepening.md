...
**Phase 2, AccountAccess service.** A new module `backend/grillmi/services/account_access.py` exposes a single class `AccountAccess` with methods that own each of `auth.py`'s flows: `authenticate(db, email, password, ip, user_agent)`, `logout(db, token)`, `request_password_reset(db, email, ip)`, `set_password(db, raw_token, new_password, ip, user_agent)`, `list_sessions(db, user_id, current_token)`, `revoke_session(db, user_id, session_id, ip)`, `delete_account(db, user_id, ip)`, plus a CLI-only `create_user_with_invitation(db, email, first_name)` that owns the user-row creation + invitation token issuance currently inlined in `admin_init.py`. The HTTP-facing methods accept `ip: str | None`; passing `None` skips per-IP rate limiting and is reserved for CLI callers running locally. Rate limiting, Argon2 verify, HIBP check, audit log writes, email sending, reset-token consumption, session row create and delete all move into this module. The private helper `_hash_token` from `auth.py` relocates to the service since it operates on token bytes, not HTTP. The route handlers in `auth.py` collapse to thin shells that translate request and response; cookie ops (`_set_session_cookie`, `_clear_session_cookie`, `_client_ip`) stay in `auth.py` because they are HTTP concerns. The admin CLI verbs `admin_init` and `admin_reset` switch to call `AccountAccess.create_user_with_invitation` and `AccountAccess.request_password_reset(ip=None)` directly so tests, CLI, and HTTP share one entry point.

**Phase 3, SyncCoordinator.** A new module `src/lib/sync/coordinator.ts` exposes the public surface today scattered across `queue.ts` and `pull.ts`: `syncNow(reason)`, `enqueueWrite(args)`, `subscribe(listener)`, `attachSync()`, `detachSyncForTests()`. The coordinator owns the invariants today scattered across `pull.ts`, `queue.ts`, and `grilladeSync.ts`: single-active-grillade enforcement (the `retireOtherActiveGrilladen` logic in `pull.ts:194-210` and the missing-server-row repair in `grilladeSync.ts:65-78`), watermark plumbing (`lastPullEpoch` reads and writes in `pull.ts`), 401 logout-and-redirect, 409 refetch-then-write-to-IDB-without-pushing, 5xx retry-keep, 4xx-other drop, debounced flush, visibility-change and online triggers. `pull.ts`, `queue.ts`, and `pushGrillade.ts` become private adapters under `src/lib/sync/_adapters/` that the coordinator drives. `debug.ts` stays in place as a low-level event recorder used by stores, runtime, and the diag page; it is not a sync primitive and does not move. `firstLogin.ts` is a one-shot bootstrap that runs at first login and bypasses the queue; it stays a separate sibling module rather than a coordinator concern. Today's external callers of `flush()` followed by `pull()` (the layout boot sequence and the diag page) collapse to `syncNow(reason)`; today's callers of `enqueueSync` (api/client, favoritesStore, settingsStore) switch to `enqueueWrite`. `onSyncApplied` is the established name for the listener registration and stays exposed alongside `subscribe` (one is a thin alias for the other) to avoid touching every consumer in this phase. Stores and `+layout.svelte` only ever import from `coordinator.ts` (re-exported through `src/lib/sync/index.ts`).

**Phase 4, GrilladeLifecycle.** A new module `src/lib/grillade/lifecycle.ts` exports a `createGrilladeLifecycle({ persistence, push, scheduler, clock })` factory that takes ports for the things today's store reaches for directly: `persistence` (the `db.ts` Grillade-row helpers), `push` (the SyncCoordinator from Phase 3), `scheduler` (the existing `schedule.ts` and `buildSessionItem`), and `clock` (a `now()` function for tests). The factory returns the full lifecycle interface: plan mutators (`addItem`, `updateItem`, `removeItem`, `reorderItems`, `loadFromMenu`, `setTargetTime`, `setPlanMode`, `setAutoMode`, `resetDraft`), session entry points (`startSession`, `startManualSession`, `startSessionItem`), session mutators (`patchItem`, `plateItem`, `unplateItem`, `forceReady`, `removeSessionItem`), timeline (`appendTimelineEvent`), end-of-grillade (`endSession`), and read accessors (`plan`, `session`, `planMode`, `autoMode`, `cookingItems`, `sessionHasStarted`, `longestCookSeconds`, `effectiveTargetEpoch`, `hasItems`, `hasActive`). `grilladeStore.svelte.ts` shrinks to a Svelte rune-aware view that wraps a single lifecycle instance and surfaces its state through `$state` and `$derived`. `grilladeSync.ts` deletes; its push-decision logic lives inside the lifecycle's wiring of the `push` port. `pull.ts`'s `refreshLocalActiveItems` becomes a coordinator pull-hook that hands rows to the lifecycle's "remote-update" entry point rather than writing directly into the store's territory.

The four phases land in this order on one branch (`cleanup/architectural-deepening`), one commit per phase, full suite green after each commit. Each phase merges to main as its own no-ff merge once the commit is green, preserving the phase boundaries in history (squash and rebase are out per project policy on never rewriting git history).
