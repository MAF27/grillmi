...
- Status: Implementation, automated walkthrough, agent-browser visual diff, and code reconciliation all complete. Awaiting Marco approval for push, merge to main, and `grillmi-deploy.yml` (Phase 9 step 6). Physical-device residue (real touch-momentum, audible tone QA, low-end Android paint smoothness) tracked under §Manual Verification. The install-app chip on Home was deliberately removed post-redesign; the only entry into A2HS is the OS-level Safari "Zum Home-Bildschirm" affordance.
...
6. Home does not render an install prompt. Users who want to install the PWA use Safari's native "Zum Home-Bildschirm" share-sheet action. (Deviation from the prototype: the original "App installieren" chip plus iOS instructional sheet shipped in Phase 7 and were removed in a follow-up because the chip felt like clutter on a screen the user sees every grilling session.)
...
**Visual-baseline scope correction:** the original Phase 0 plan was to capture prototype screenshots and use them as Playwright `toHaveScreenshot` baselines. That conflates regression-detection with design-match validation: Playwright's snapshot diff fires on legitimate React-vs-Svelte rendering differences (font load states, anti-aliasing, mock vs real data), so prototype PNGs as baselines are a false gate. Validation that the rebuilt app matches the Glühen direction lives in Phase 9's agent-browser visual diff at 390×844 (Claude drives Chromium against both the prototype HTML and the dev server, captures screenshots per state, lists drift, and reconciles in code). The earlier scaffold under `tests/e2e/visual-baselines/gluehen/` plus `gluehen-visual.spec.ts` plus the `test:e2e:visual` package script was removed when the gap was identified.
...
3. [x] No install affordance on Home. (Originally shipped a dismissible "App installieren" chip backed by `beforeinstallprompt` plus an iOS instructional sheet; both were removed in a follow-up. Users install via Safari's "Zum Home-Bildschirm" share action. Removed: the chip, the iOS sheet, the `gluehen.installChipDismissed` localStorage key, the `beforeinstallprompt` listener in `+layout.svelte`, and the `Window.installPromptEvent` type in `app.d.ts`.)
...
#### Phase 9: QA, automated walkthrough, visual diff, and ship

Every step in this phase is a Claude task. Marco's only role is approval at step 6.

1. [x] Run `pnpm test:unit && pnpm test:components && pnpm lint && pnpm build`. All green.
2. [x] Write `tests/e2e/walkthrough.spec.ts`. The spec runs at the 390×844 mobile viewport and drives the entire user-visible surface end to end: Home (wordmark, hero copy, three CTAs, recent-Menüs strip hidden when empty) → Plan (segmented control with three labels, eat-time card empty state, AddItemSheet category → cut → specs walk including a thickness-stepper round-trip, eating-time picker dialog opens on populated-card tap and Übernehmen commits the chosen HH:MM, "Als Menü speichern" dialog with placeholder copy and IDB roundtrip) → Home (recent-Menü pill renders post-save) → Menüs (row name + meta render, swipe-left reveals Delete, inline rename commits on Enter and persists across reload) → Plan manual (Los → cooking → ready via `page.clock.fastForward()`, Anrichten button appears, sticky CTA hidden) → `/session` direct-nav while in manual mode bounces back to `/plan` → live Session (master countdown, 92 px ring, Live dot, wake-lock chip, Beenden button) → Alarm banner fires on the scheduled tick, gradient + queue pill render, dismiss closes → drive every item to plated, advance the fake clock by 90 s, assert session is still active, tap Beenden routes to Home → Settings (System/Hell/Dunkel toggles `data-theme`, Auflegen accordion expands, five tone names render, picking a tone updates the head label, audio element gets the correct `src` and plays without error for each non-Lautlos tone, Lautlos plays no audio) → offline + font-load (`context.setOffline(true)`, reload, assert `getComputedStyle(document.body).fontFamily` resolves through Barlow Condensed for display and Inter for body, no fallback names leak through). Use `page.clock.install()` for every wait greater than 1 s.
3. [x] Run `pnpm test:e2e`. Reconcile any flake by stabilizing locators or clock usage; do not weaken assertions. All green.
4. [x] Drive Chromium via Playwright at 390×844 (dark colorScheme) against (a) `.tmp/grillmi-design/design_handoff_grillmi_redesign/design/Grillmi Redesign.html` served from `python3 -m http.server 8000` and (b) the preview server at `http://localhost:4173`. `tests/e2e/visual-capture.spec.ts` runs with `VISUAL_CAPTURE=1`, patches the prototype's TWEAKS literal per state via route interception, and writes screenshot pairs to `.tmp/visual-diff/{prototype,app}/`. Reconciled drift in code: removed UI em-dashes from `src/routes/+page.svelte` hero subtext, `src/routes/plan/+page.svelte` CTA + hint + warning copy; added the right-aligned "X STÜCK" item count next to the GRILLSTÜCKE eyebrow on Plan. Remaining differences are deliberate Fidelity-contract deviations already documented (wake-lock chip on Session, "Als Menü speichern" on Plan, German doneness labels, no install chip on Home).
5. [x] Run `pnpm test:e2e` once more after step 4 fixes. 36 green, 16 visual-capture tests skipped without `VISUAL_CAPTURE=1`.
6. [ ] Pause and request explicit Marco approval. On approval: push `feature/gluehen` to the remote, fast-forward merge `feature/gluehen` into `main` locally, push `main`, run `~/dev/ansible/playbooks/applications/grillmi-deploy.yml`.
...
2. [x] `home.spec.ts` (new): recent-Menüs strip hidden when empty, strip + pill render after seeding a Menü, and the brand + hero + three CTAs render on Home. No install affordance is rendered, so no `beforeinstallprompt` assertions.
...
4. [x] `eating-time-picker.spec.ts` (new): the picker dialog opens on populated-eatcard tap and the Abbrechen path closes it without committing. CSS-scroll-snap flicking on a real touchscreen is covered by §Manual Verification step 2; Playwright's mouse synthesis does not reproduce real touch-momentum.
5. [x] `auto-end-removed.spec.ts`: implemented at the unit level (`tests/unit/sessionStore.test.ts` no longer has any auto-end timer assertion; `endSession` now only fires on explicit user action). The Phase 9 walkthrough spec asserts the live behaviour via fake-clock advance plus plated-no-auto-end + Beenden routing.
...
7. [x] `gluehen-visual.spec.ts` (new): removed. See the Phase 0 "Visual-baseline scope correction" note: a Playwright `toHaveScreenshot` against prototype PNGs is the wrong tool for "matches the Glühen direction" validation. The visual gate moves to Phase 9 step 4 (Claude-driven agent-browser side-by-side at 390×844 against the prototype HTML and the dev server) before merge.

### Manual Verification (physical-device residue only)

Everything that can be exercised in a headless browser is covered by `tests/e2e/walkthrough.spec.ts` (Phase 9 step 2) and the agent-browser visual diff (Phase 9 step 4). The three items below are the only checks that physically require Marco plus a real device: real touch-momentum gestures, audible tone judgment on real speakers, and paint smoothness on a low-end Android.

1. [x] Real iOS device: on Plan with a populated eating-time card, tap the card. The bottom-sheet picker opens. Flick the hour column with a finger; momentum scroll lands on a snapped row with a subtle ember underline. Repeat with the minute column. Tap Übernehmen and confirm the eat-time card updates.
2. [x] Listen to each chime (Glut, Funke, Kohle, Klassik) on phone-grade speakers. Confirm each tone has a smooth fade-out without abrupt cut, all four normalize to a consistent perceived loudness, and each tone is sonically distinct from the other three. Confirm Lautlos plays nothing.
3. [ ] On a low-end Android device (or Chrome DevTools 4× CPU throttle on the dev machine), open Home, then Plan, then drive a Session that triggers the alarm banner. The GlowGrates radial glow on Home renders without visible banding; the AlarmBanner pulse animation runs without dropping frames; scrolling Plan with several item rows stays smooth.
