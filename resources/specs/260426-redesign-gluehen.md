...
- Status: Implementation, automated walkthrough, agent-browser visual diff, and code reconciliation all complete. Awaiting Marco approval for push, merge to main, and `grillmi-deploy.yml` (Phase 9 step 6). Physical-device residue (iOS A2HS, real touch-momentum, audible tone QA, low-end Android paint smoothness) tracked under §Manual Verification.
...
2. [x] `home.spec.ts` (new): recent-Menüs strip hidden when empty, strip + pill render after seeding a Menü, and the brand + hero + three CTAs render on Home. iOS install-chip persistence is covered by §Manual Verification step 1 (real-device A2HS); Chromium does not fire `beforeinstallprompt` deterministically in headless.
...
Everything that can be exercised in a headless browser is covered by `tests/e2e/walkthrough.spec.ts` (Phase 9 step 2) and the agent-browser visual diff (Phase 9 step 4). The four items below are the only checks that physically require Marco plus a real device: an iOS Safari install flow, real touch-momentum gestures, audible tone judgment on real speakers, and paint smoothness on a low-end Android.

1. [ ] Real iOS device, Safari, non-standalone: open Grillmi. The "App installieren" chip renders above the hero. Tap the chip; the inline instructional sheet shows the Add to Home Screen icon and steps. Run through the iOS A2HS flow. Launch the installed PWA from the home screen and confirm it opens in standalone mode and the chip no longer renders.
2. [ ] Real iOS device: on Plan with a populated eating-time card, tap the card. The bottom-sheet picker opens. Flick the hour column with a finger; momentum scroll lands on a snapped row with a subtle ember underline. Repeat with the minute column. Tap Übernehmen and confirm the eat-time card updates.
3. [ ] Listen to each chime (Glut, Funke, Kohle, Klassik) on phone-grade speakers. Confirm each tone has a smooth fade-out without abrupt cut, all four normalize to a consistent perceived loudness, and each tone is sonically distinct from the other three. Confirm Lautlos plays nothing.
4. [ ] On a low-end Android device (or Chrome DevTools 4× CPU throttle on the dev machine), open Home, then Plan, then drive a Session that triggers the alarm banner. The GlowGrates radial glow on Home renders without visible banding; the AlarmBanner pulse animation runs without dropping frames; scrolling Plan with several item rows stays smooth.
