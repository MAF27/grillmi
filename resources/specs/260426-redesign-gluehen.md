...
1. [ ] Real iOS device: on Plan with a populated eating-time card, tap the card. The bottom-sheet picker opens. Flick the hour column with a finger; momentum scroll lands on a snapped row with a subtle ember underline. Repeat with the minute column. Tap Übernehmen and confirm the eat-time card updates.
2. [ ] Listen to each chime (Glut, Funke, Kohle, Klassik) on phone-grade speakers. Confirm each tone has a smooth fade-out without abrupt cut, all four normalize to a consistent perceived loudness, and each tone is sonically distinct from the other three. Confirm Lautlos plays nothing.
