...
7. Replacement of the existing chime audio with new recordings before this redesign ships. New audio is sourced from freesound.org during this work and lands in the same branch.
...
The Settings tone migration replaces `chime-1.mp3` through `chime-8.mp3` in `/static/sounds/` with the five new files `glut.mp3`, `funke.mp3`, `kohle.mp3`, `klassik.mp3`, plus a no-op marker for `lautlos`. Audio is sourced from freesound.org during this branch's work: I propose four candidate tracks per tone, you preview and approve, the chosen file is downloaded under its CC0 or CC-BY license, normalized to `-14 LUFS` and trimmed to under 1.5 seconds, encoded as MP3 at 96 kbps mono, and committed under the tone's name. The license metadata for each tone is recorded in `static/sounds/README.md`. The `soundAssignmentSchema` default migrates from `{ putOn: 'chime-1', flip: 'chime-2', done: 'chime-3' }` to `{ putOn: 'glut', flip: 'funke', done: 'klassik' }`. Existing user settings that reference `chime-N` are migrated on read in the settings store: any value not in the new five-tone set falls back to the new default for that event.
...
5. **Freesound.org licensing is workable for this scope.** Audio under CC0 or CC-BY 4.0 can be bundled and ship with attribution in `static/sounds/README.md`. Five short tones at 96 kbps mono come to under 100 kB total, which is acceptable PWA payload. Reference: https://freesound.org/help/faq/#what_can_i_do_with_freesound_sounds.
...
| Audio sourced from freesound.org has unclear licensing | Only files marked CC0 or CC-BY 4.0 are eligible; license URL plus file ID plus author plus license SPDX identifier are recorded per tone in `static/sounds/README.md`; CC-BY attribution renders in Settings via an "Über die Töne" disclosure if any chosen tone is CC-BY. |
...
1. [ ] Source five tones from freesound.org per the audio-production behavior. For each of Glut, Funke, Kohle, Klassik, identify four CC0 or CC-BY 4.0 candidates, present them to Marco for preview-and-approve, download the chosen file, normalize to -14 LUFS, trim under 1.5 seconds, encode as MP3 96 kbps mono, commit under `static/sounds/{tone}.mp3`. Lautlos has no audio file.
2. [ ] Add `static/sounds/README.md` recording the source URL, freesound user, license SPDX (`CC0-1.0` or `CC-BY-4.0`), and license URL per chosen tone.
3. [ ] Delete `static/sounds/chime-1.mp3` through `static/sounds/chime-8.mp3`.
4. [ ] In `src/lib/schemas/index.ts`, update `soundAssignmentSchema`'s default to `{ putOn: 'glut', flip: 'funke', done: 'klassik' }`. Update the union of allowed values from `chime-1...chime-8` to `glut`, `funke`, `kohle`, `klassik`, `lautlos`.
5. [ ] In `src/lib/stores/settingsStore.svelte.ts`, add a read-time fallback: when a persisted value is not in the new five-tone set, replace it with the new default for that event in the in-memory state and rewrite to IDB on the next setSound. This handles existing users gracefully.
6. [ ] Rewrite `src/routes/settings/+page.svelte` to match the prototype's three-section layout. Section 1 "Darstellung" uses `SegmentedControl` for System/Hell/Dunkel. Section 2 "Signale" lists three event rows (Auflegen, Wenden, Fertig) as collapsible accordions; each expands to the five-tone radio list with descriptions and a play-preview button. Section 3 is a full-width vibration toggle row with sub-line copy "zusätzlich zum Ton".
7. [ ] Wire the play-preview button to a small audio playback helper that loads `/sounds/{tone}.mp3` and plays it once, ignoring Lautlos. Re-use the existing alarm audio playback path if present; otherwise add a minimal `previewTone(toneId)` helper in the settings page.
